...
export function getSemanticIndexStats(index: ScriptSourceIndex, settings: SemanticSettings): {
    chunkCount: number;
...
async function embedMissingChunks(session: SemanticVectorSession, sessionCacheKey: string, chunks: ScriptChunk[], settings: SemanticSettings, onProgress?: (progress: SemanticSearchProgress) => void): Promise<void> {
    pruneStaleVectors(session, chunks);
    const uniqueChunks = uniqueChunksByEmbedding(chunks);
...
export async function semanticSearchScripts(index: ScriptSourceIndex, settings: SemanticSettings, query: string, limit: number, minScore?: number, onProgress?: (progress: SemanticSearchProgress) => void): Promise<SemanticSearchOutput> {
    const chunks = buildChunks(index.scripts);
    onProgress?.({
...
export async function semanticIndexCodebase(index: ScriptSourceIndex, settings: SemanticSettings, onProgress?: (progress: SemanticSearchProgress) => void): Promise<{
    chunkCount: number;
    embeddedChunks: number;
    sourceIndexComplete: boolean;
    isPartialIndex: boolean;
...
export function getScriptIndexStatus(debugId: string, index: ScriptSourceIndex, settings: SemanticSettings): {
    totalChunks: number;
    embeddedChunks: number;
