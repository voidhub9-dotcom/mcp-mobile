...
import { relayClients, relayRequestOrigin, requestToClientId, SendToClient, } from "../../bridge/handlers/shared/communication.js";
import { formatActiveClientListForTool, resolveTargetClient, setActiveClientId, } from "../../bridge/handlers/shared/registry.js";
    ...
    ws.on("message", (rawData) => {
        try {
            ...
            if (message.type === "set-active-client" && message.id) {
                const requestedClientId = typeof message.targetClientId === "string" ? message.targetClientId : "";
                ...
                ws.send(JSON.stringify({
                    id: message.id,
                    output: `Active client set to ${target.clientId} ` +
