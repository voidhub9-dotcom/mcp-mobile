...
export async function POST(req: IncomingMessage, res: ServerResponse): Promise<void> {
    try {
        ...
        if (!target) return send(res, 404, { error: "No connected client was found." });
        ...
        let capture: { imageBase64?: unknown; mimeType?: unknown; error?: unknown; success?: unknown };
        try {
        ...
        return send(res, 500, { error: `Screenshot request failed: ${(error as Error).message || error}` });
    }
}
