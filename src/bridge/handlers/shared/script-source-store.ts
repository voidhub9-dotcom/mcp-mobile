...
function updateScriptHashIndex(store: ClientScriptSourceStore, script: StoredScriptSource): void {
...
function canActivateScriptSource(store: ClientScriptSourceStore, input: UpsertScriptSourcesInput, inputRevision: number | undefined, debugId: string): boolean {
...
function getScriptSourceIndexSummary(identity: ScriptSourceStoreIdentity, store: ClientScriptSourceStore): ScriptSourceIndexSummary {
...
function applyMappingRevision(store: ClientScriptSourceStore, input: UpsertScriptSourcesInput): MappingRevisionDecision {
...
export function upsertScriptSources(identity: ScriptSourceStoreIdentity, input: UpsertScriptSourcesInput): ScriptSourceUpsertResult {
...
export function getCachedScriptSourcesByScriptHash(identity: ScriptSourceStoreIdentity, scriptHashes: unknown[]): CachedScriptSourceByHash[] {
    const store = getOrCreateStore(identity);
