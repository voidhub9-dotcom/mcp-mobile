...
export default function register(server: McpServer): void {
  server.registerTool(
    "get-client-capabilities",
    {
      ...
      description:
        "Returns the capability profile of the active Roblox client, including executor name, platform (mobile/desktop), and which APIs are available (loadstring, request, WebSocket, decompile, getscripts, firesignal, etc.). Use this to determine which tools will work on the connected client, especially for mobile executors with limited API support.",
    },
    async () => {
      const clientId = getActiveClientId();
      const target = resolveTargetClient(clientId);
      if (!target) return NO_CLIENT_ERROR;
      ...
      if (target.capabilities) {
        ...
        const toolChecks: Array<{ tool: string; available: boolean; reason?: string }> = [
