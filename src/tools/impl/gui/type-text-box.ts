...
export default function register(server: McpServer): void {
    server.registerTool("type-text-box", {
        title: "Type into a TextBox",
        description: "Enter text into a Roblox TextBox by path. Can simulate keystrokes or set Text directly based on useKeyPress.",
    ...
    }, async ({ path, text, enter, useKeyPress }) => sendAndWait({
        type: "type-text-box",
        data: { path, text, string: text, enter, useKeyPress },
        failureField: "error",
        failureMessage: (response) => "Failed to type into TextBox: " + describeResponse(response),
        successMessage: (response) => (response.output as string | undefined) || "Successfully typed into TextBox.",
    }));
}
