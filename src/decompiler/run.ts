import { DEFAULT_DECOMPILER_RUNTIME_SETTINGS, DEFAULT_PROVIDER_TIMEOUTS_MS, isCustomDecompilerProviderId, isDecompilerProviderId, type DecompilerProviderId, type DecompilerProviderSettings, type DecompilerRuntimeSettings, type DecompilerSettings, } from "./settings.js";
import { getDecompilerProviderStatus, recordDecompilerProviderFailure, recordDecompilerProviderSuccess, shouldSkipDecompilerProvider, } from "./health.js";
...
function providerDisplayName(id: DecompilerProviderId, provider: DecompilerProviderSettings): string {
    if (id === "luaexpert")
        return "lua.expert";
    if (id === "shiny")
        return provider.options.mode === "hosted" ? "Shiny hosted" : "Shiny";
    if (id === "oracle")
        return "Oracle";
    if (id === "konstant")
        return "Konstant";
...
function cleanDisabledProviders(value: unknown[] | undefined): Set<DecompilerProviderId> {
    ...
    for (const item of value) {
        if (isDecompilerProviderId(item))
            disabled.add(item);
    }
    return disabled;
}
function orderProvidersForRequest(candidates: DecompilerProviderId[], runtime: DecompilerRuntimeSettings, requestedProvider: DecompilerProviderId | null, clientId = "server"): DecompilerProviderId[] {
    ...
    if (runtime.loadBalanceSlowProviders === false ||
        !firstCandidate ||
...
async function fetchText(url: string, init: Omit<RequestInit, "signal">, timeoutMs: number): Promise<ProviderRunResult> {
...
async function runLuaExpert(provider: DecompilerProviderSettings, bytecodeBase64: string, timeoutMs: number): Promise<ProviderRunResult> {
    return withLuaExpertRateLimit(() => fetchText(resolveProviderEndpoint(provider.endpoint), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ script: bytecodeBase64 }),
    }, timeoutMs));
}
async function runPlainBase64(provider: DecompilerProviderSettings, bytecodeBase64: string, timeoutMs: number): Promise<ProviderRunResult> {
    return fetchText(resolveProviderEndpoint(provider.endpoint), {
        method: "POST",
        headers: { "Content-Type": "text/plain" },
        body: bytecodeBase64,
    }, timeoutMs);
}
async function runPlainBytecode(provider: DecompilerProviderSettings, bytecode: Buffer, timeoutMs: number): Promise<ProviderRunResult> {
    return fetchText(resolveProviderEndpoint(provider.endpoint), {
        method: "POST",
        headers: { "Content-Type": "text/plain" },
...
async function runOracle(provider: DecompilerProviderSettings, bytecodeBase64: string, timeoutMs: number): Promise<ProviderRunResult> {
    if (!provider.apiKey) {
        return {
            ok: false,
...
function customOptionString(provider: DecompilerProviderSettings, key: string, fallback: string): string {
    const value = provider.options[key];
    return typeof value === "string" && value.trim() ? value.trim() : fallback;
}
function customOptionTemplate(provider: DecompilerProviderSettings, key: string, fallback: string): string {
    const value = provider.options[key];
...
function customHeaders(provider: DecompilerProviderSettings): Record<string, string> {
    ...
    if (provider.apiKey) {
        ...
        const existingHeader = Object.keys(headers).find((name) => name.toLowerCase() === header.toLowerCase());
        if (existingHeader)
...
function customTemplateVariables(provider: DecompilerProviderSettings, bytecode: Buffer, bytecodeBase64: string): Record<string, CustomTemplateValue> {
    ...
    for (const item of configured) {
        ...
        if (typeof name !== "string")
            continue;
        if (value === "bytecode")
            values[name] = bytecode;
        else if (value === "base64")
            values[name] = bytecodeBase64;
    }
    return values;
}
function renderCustomTemplate(template: string, values: Record<string, CustomTemplateValue>): string {
...
function renderCustomHeadersTemplate(template: string, values: Record<string, CustomTemplateValue>): Record<string, string> {
    ...
    for (const [name, value] of Object.entries(parsed)) {
        if (typeof value !== "string")
            throw new Error("Rendered request header values must be strings.");
        if (name.trim())
            headers[name.trim()] = value;
    }
    return headers;
}
function renderCustomBodyTemplate(template: string, values: Record<string, CustomTemplateValue>): {
    body: BodyInit;
    binary: boolean;
} {
...
async function runCustom(provider: DecompilerProviderSettings, bytecode: Buffer, bytecodeBase64: string, timeoutMs: number): Promise<ProviderRunResult> {
    ...
    const result = await fetchText(resolveProviderEndpoint(effectiveProvider.endpoint), { method: "POST", headers, body }, timeoutMs);
...
async function runProvider(options: {
    ...
    if (isCustomDecompilerProviderId(options.id)) {
        return runCustom(options.provider, options.bytecode, options.bytecodeBase64, options.timeoutMs);
    }
    return {
        ok: false,
...
export function resolveDecompilerProviders(settings: DecompilerSettings, options: Pick<DecompileInput, "requestedProvider" | "disabledProviders" | "clientId"> = {}): ResolvedDecompilerProviders {
...
export async function decompileBytecode(settings: DecompilerSettings, input: DecompileInput): Promise<DecompileResult> {
    const runtime = runtimeOrDefault(settings.runtime);
    ...
    for (const id of resolved.orderedProviders) {
        ...
        attemptedProviders.push(id);
        const providerTimeoutMs = Math.min(runtime.providerTimeoutsMs?.[id] ?? DEFAULT_PROVIDER_TIMEOUTS_MS[id] ?? 6000, remainingMs);
