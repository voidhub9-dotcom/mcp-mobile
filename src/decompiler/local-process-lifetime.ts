...
import { commandContainsToken, localProcessOperations, processSnapshotMatches, type ProcessOperations, type ProcessSnapshot, } from "./local-process-operations.js";
...
export function acquireProcessLease(pid: number, commandToken: string, paths: LifetimePaths = LOCAL_PROCESS_LIFETIME_PATHS, operations: ProcessOperations = DEFAULT_PROCESS_OPERATIONS): boolean {
...
function readTrackedProcesses(directory: string, operations: ProcessOperations): Array<TrackedProcess & {
    file: string;
}> {
    let entries: string[];
    try {
    ...
    const live: Array<TrackedProcess & {
        file: string;
    }> = [];
...
export function terminateTrackedProcess(tracked: TrackedProcess, operations: ProcessOperations = DEFAULT_PROCESS_OPERATIONS, policy: TerminationPolicy = DEFAULT_TERMINATION_POLICY, shouldCancel: () => boolean = () => false): boolean {
    if (!snapshotMatches(tracked, operations.snapshot(tracked.pid)))
        return true;
    if (shouldCancel())
        return false;
...
export function releaseProcessLease(pid: number, paths: LifetimePaths = LOCAL_PROCESS_LIFETIME_PATHS, operations: ProcessOperations = DEFAULT_PROCESS_OPERATIONS, terminationPolicy: TerminationPolicy = DEFAULT_TERMINATION_POLICY): number {
...
export function shutdownManagedProvidersIfNoLeases(paths: LifetimePaths = LOCAL_PROCESS_LIFETIME_PATHS, operations: ProcessOperations = DEFAULT_PROCESS_OPERATIONS, terminationPolicy: TerminationPolicy = DEFAULT_TERMINATION_POLICY): number {
...
export function trackLocalDecompilerProcess(provider: LocalProviderId, pid: number, binaryPath: string, options: {
    paths?: LifetimePaths;
