...
export function normalizeDashboardSettings(value, fallback = DEFAULT_DASHBOARD_SETTINGS) {
    ...
    return {
        ...
        backgroundMedia: {
            type: normalizeEnum(backgroundMedia.type, ["none", "image", "video"], fallback.backgroundMedia.type),
            url: normalizeMediaUrl(backgroundMedia.url, fallback.backgroundMedia.url),
            fit: normalizeEnum(backgroundMedia.fit, ["cover", "contain"], fallback.backgroundMedia.fit),
            opacity: normalizeOpacity(backgroundMedia.opacity, fallback.backgroundMedia.opacity),
        },
    };
}
export async function dashboardSettingsExist() {
    try {
...
export async function saveDashboardSettings(input) {
    if (!isObject(input))
