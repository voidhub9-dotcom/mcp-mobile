...
export function writeProtectedResourceMetadata(req, res) {
    if (!oauthEnabled()) {
...
export function writeAuthorizationServerMetadata(req, res) {
    if (!oauthEnabled()) {
        writeJson(res, 404, { error: "OAuth is not enabled." });
        return;
    }
...
function parseRegisteredClient(value) {
    if (!value || typeof value !== "object")
        return null;
    const record = value;
    const redirectUris = Array.isArray(record.redirect_uris)
        ? record.redirect_uris.filter((uri) => typeof uri === "string" && validRedirectUri(uri))
        : [];
...
export function registerClient(req, res, requestBody) {
    if (!oauthEnabled()) {
        writeJson(res, 404, { error: "OAuth is not enabled." });
        return;
    }
...
function canonicalizeResource(value) {
    try {
        const url = new URL(value);
        url.search = "";
        url.hash = "";
...
function validateAuthorizationRequest(req, url) {
...
function writeAuthorizationError(res, status, error, description) {
    res.writeHead(status, {
...
export function renderAuthorizationPage(req, res, url) {
    if (!oauthEnabled()) {
...
function cleanExpiredAuthorizationCodes() {
    const now = Date.now();
...
export function approveAuthorization(req, res, body) {
...
function issueTokens(clientId, resource, scope) {
...
export function exchangeToken(req, res, body) {
    if (!oauthEnabled()) {
        writeJson(res, 404, { error: "OAuth is not enabled." });
        return;
    }
...
export function isValidMcpAccessToken(req, authorization) {
...
export function writeMcpUnauthorized(req, res) {
...
export function getStaticAuthToken() {
    return MCP_AUTH_TOKEN;
}
export function staticTokenMatches(value) {
