    ...
    try {
        ...
        if (data.jobId && data.progressUrl) {
            ...
            while (Date.now() < deadline) {
                ...
                const job = (await progressResp.json());
                if (job.status === "done") {
                    return toolTextResponse(String(job.result ?? "Done."), outputOptions);
                }
                if (job.status === "failed") {
                    return toolTextResponse(`Failed: ${job.error ?? "Unknown error"}`, outputOptions, true);
                }
            }
        ...
        return toolTextResponse(`Failed to relay to primary: ${err.message || err}`, outputOptions, true);
    }
}
export function normalizeMaxOutputChars(value, fallback = DEFAULT_TOOL_OUTPUT_CHAR_LIMIT) {
    const parsed = Number(value);
    if (!Number.isFinite(parsed))
        return fallback;
    return Math.min(HARD_TOOL_OUTPUT_CHAR_LIMIT, Math.max(1000, Math.floor(parsed)));
}
export function formatToolText(text, options = {}) {
...
export function toolTextResponse(text, options = {}, isError = false) {
    return {
...
export async function sendAndWait(options) {
    ...
    const text = options.successMessage?.(response) ?? response.output;
