...
function formatSemanticResults(query, output) {
    const { results, chunkCount, embeddedChunks, sourceIndexComplete, isPartialIndex } = output;
    const header = `${results.length} semantic ${results.length === 1 ? "match" : "matches"} ` +
    ...
    if (results.length > 0) {
        parts.push(results
            ...
            const signals = result.features.length > 0
                ...
                : "";
            return (`${index + 1}. [${result.path}] lines ${result.startLine}-${result.endLine} ` +
...
export default function register(server) {
    server.registerTool("semantic-search-scripts", {
        title: "Semantically search scripts in the game",
        description: "Find decompiled Roblox scripts by behavior using enriched semantic cards plus exact lexical signals. Use when exact identifiers are unknown; use script-grep for precise text or regex.",
        ...
        try {
            if (indexOnly || requireFullIndex) {
                const { chunkCount, embeddedChunks, sourceIndexComplete } = await semanticIndexCodebase(indexResult.index, settings);
                if (indexOnly) {
                    return toolTextResponse(semanticIndexReadyMessage({ chunkCount, embeddedChunks, sourceIndexComplete }, indexResult.index), { maxOutputChars });
                }
            }
            const output = await semanticSearchScripts(indexResult.index, settings, query, normalizeLimit(limit), minScore);
            if (requireFullIndex && output.isPartialIndex) {
                return toolTextResponse("Semantic search did not complete a full index; refusing partial results. Rerun with requireFullIndex=false only if partial results are acceptable.", {}, true);
            }
