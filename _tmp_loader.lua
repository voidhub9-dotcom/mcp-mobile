...
do
    ...
    for _, holder in ipairs({ env.__VOIDHUB_BX, env.__VOIDHUB_TOP }) do
    ...
    env.__VOIDHUB_TOP = {}
    env.__VOIDHUB_GEN = (tonumber(env.__VOIDHUB_GEN) or 0) + 1
end

VoidHubTop = ((type(getgenv) == "function" and getgenv()) or _G).__VOIDHUB_TOP

VoidHubLoadT = { start = os.clock() }


VoidHubRealPrint = VoidHubRealPrint or print
print = function(...)
    local env = (type(getgenv) == "function" and getgenv()) or _G
    if env.VoidHubDebug then VoidHubRealPrint(...) end
end
local function trace(msg)
    if type(writefile) == "function" then
        pcall(writefile, "VoidHub_trace.txt", tostring(msg) .. "\n")
    end
end


VoidHubSplash = { step = function() end, discord = function() end,
...
;(function()
    local ok = pcall(function()
        ...
        local old = parent:FindFirstChild("VoidHubSplash")
        if old then old:Destroy() end

        local gui = mk("ScreenGui", {
            Name = "VoidHubSplash", DisplayOrder = 999997, IgnoreGuiInset = true,
        ...
        local title = mk("TextLabel", {
            ...
            TextXAlignment = Enum.TextXAlignment.Left, Text = "VoidHub",
        }, card)
        local sub = mk("TextLabel", {
            ...
            TextXAlignment = Enum.TextXAlignment.Left, Text = "Steal An Egg",
        }, card)
        ...
        local function accept(opened)
            ...
            if opened then
                ...
                tryDiscordApp()
                print("[VOIDHUB] Discord: " .. INVITE_URL)
        ...
        VoidHubSplash.step = step
        VoidHubSplash.discord = discord
        VoidHubSplash.whenClosed = function(fn)
            if closedDone then pcall(fn) else onClosed[#onClosed + 1] = fn end
        end
        VoidHubSplash.fail = function(why)
            if finished then return end
            status.Text = tostring(why)
            status.TextColor3 = Color3.fromRGB(240, 110, 110)
            close(4)
        end
        VoidHubSplash.done = function()
            if finished then return end
            step("Ready", 1)
    ...
    if not ok then warn("[VOIDHUB] loading screen unavailable") end
end)()
VoidHubSplash.step("Loading interface", 0.08)
...
local ThemeManager
do
    local UI_ROOT = "https://raw.githubusercontent.com/deividcomsono/Obsidian/main/"
    local UI_URL = UI_ROOT .. "Library.lua"
    local lastErr
    for attempt = 1, 4 do
        local ok, res = pcall(function() return game:HttpGet(UI_URL) end)
        ...
        warn(("[VOIDHUB] UI host attempt %d/4 failed: %s"):format(attempt, tostring(lastErr)))
        task.wait(0.35 * attempt)
    end

    if not Library then
        warn("[VOIDHUB] Could not load the UI library. The menu host is down, not your executor.")
        warn("[VOIDHUB] Last error: " .. tostring(lastErr))
        VoidHubSplash.fail("Menu host is down - try again in a minute")
        return
    end

    pcall(function()
        SaveManager = loadstring(game:HttpGet(UI_ROOT .. "addons/SaveManager.lua"))()
    end)
    pcall(function()
        ThemeManager = loadstring(game:HttpGet(UI_ROOT .. "addons/ThemeManager.lua"))()
    end)
end


VoidHubLoadT.lib = os.clock()
VoidHubSplash.step("Waiting for the game", 0.3)
...
if not net then
    VoidHubSplash.fail("Networking not found - are you in Steal An Egg?")
    return
end
VoidHubSplash.step("Hooking networking", 0.38)
...
VoidHubSplash.step("Loading egg data", 0.45)
...
if not (EggState and GuardAreas) then
    VoidHubSplash.fail("Game not fully loaded - rejoin and try again")
    return
end
VoidHubSplash.step("Mapping guard areas", 0.55)


VoidHubLoadT.game = os.clock()
VoidHubSplash.discord()
VoidHubLoadT.discord = os.clock()

VoidHubSplash.step("Building menu", 0.65)


pcall(function()
    local env = (type(getgenv) == "function" and getgenv()) or _G
    if env.__VOIDHUB_WINDOW and not env.__VOIDHUB_WINDOW.unloaded then
        env.__VOIDHUB_WINDOW:Unload()
    end
end)
...
local _rawWindow = Library:CreateWindow({
    Title               = "VoidHub",
    Footer              = "discord.gg/getvoidhub",
...
local Window = _rawWindow

local Webhook = { Url = "", Enabled = false, AllNotices = false, LastSend = 0 }
...
function Webhook:Send(title, description, force)
    if not force and not self.Enabled then return false, "disabled" end
    local url = tostring(self.Url or ""):match("^%s*(.-)%s*$")
    if not url:match("^https://[^/]+/api/webhooks/%d+/[%w_%-]+") then
        return false, "invalid Discord webhook URL"
    end
    local req = executorRequest()
    if type(req) ~= "function" then return false, "HTTP request is unavailable" end
    if os.clock() - self.LastSend < 1.5 then return false, "rate limited" end
    self.LastSend = os.clock()
    local body = game:GetService("HttpService"):JSONEncode({
        username = "VoidHub",
        embeds = {{
            title = tostring(title or "VoidHub"),
            description = tostring(description or "Activity update"),
            color = 11184810,
            fields = {
                { name = "Player", value = LocalPlayer.DisplayName .. " (@" .. LocalPlayer.Name .. ")", inline = true },
                { name = "Game", value = tostring(game.Name), inline = true },
            },
            footer = { text = "VoidHub activity webhook" },
    ...
    return ok and code and code >= 200 and code < 300,
        ok and (code and ("HTTP " .. code) or "no status") or tostring(response)
end

function VoidHubWebhookEvent(title, description)
    task.spawn(function() Webhook:Send(title, description, false) end)
end

local _controls = {}

local function _mkProxy(ctrlObj)
    return ctrlObj
end

local function _reg(flag, ctrlObj)
    if flag and ctrlObj then _controls[flag] = _mkProxy(ctrlObj) end
end


local __gbHome,  __gnHome  = nil, 0
local __gbSteal, __gnSteal = nil, 0
local __gbFarm,  __gnFarm  = nil, 0
local __gbEvent, __gnEvent = nil, 0
local __gbMisc,  __gnMisc  = nil, 0
local __gbCfg,   __gnCfg   = nil, 0


local function _addGB(rawTab, gnRef, gbRef, name)
    gnRef[1] = gnRef[1] + 1
    if gnRef[1] % 2 == 1 then
        gbRef[1] = rawTab:AddGroupbox({Side="Left", Name=name})
    else
        gbRef[1] = rawTab:AddGroupbox({Side="Right", Name=name})
    end
end

;((type(getgenv) == "function" and getgenv()) or _G).__VOIDHUB_WINDOW = {
    controls = _controls, unloaded = false,
...
do
    ...
    function VoidHubStealDone(name, pick)
        ...
        task.delay(1.2, function()
            ...
            queue[#queue + 1] = msg
            if type(VoidHubWebhookEvent) == "function" then
                VoidHubWebhookEvent("Egg collected", msg)
            end
    ...
    function VoidHubPickPrompt(msg)
    ...
    local lastNote = {}
    function VoidHubNote(key, msg, cooldown)
        local now = os.clock()
        key = tostring(key)
    ...
    task.spawn(function()
        local __gen = (getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN
        local lastText, lastAt = nil, -100
        while true do
            if ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) ~= __gen then return end
            if #queue > 0 then
                task.wait(0.35)
                ...
                if not (text == lastText and os.clock() - lastAt < 4) then
                    ...
                    local ok, err = pcall(function()
                        Library:Notify({ Title = "VoidHub", Description = text, Time = 3 })
                    end)
                    if not ok then
                    ...
                    if Webhook.AllNotices then
                        task.spawn(function() Webhook:Send("VoidHub notice", text, false) end)
                    end
                end
            end
            task.wait(0.1)
...
local BX = {
    ...
    reassertCount = 0,
    REASSERT_GAP = 2.5,
    SWAP_ATTR = "VoidHubStealHum",
}


((type(getgenv) == "function" and getgenv()) or _G).__VOIDHUB_BX = BX
...
function BX.armKickLog()
    if _G.__VOIDHUB_KICKLOG then return end
    _G.__VOIDHUB_KICKLOG = true
                ...
                for _, d in ipairs(cg:GetDescendants()) do
                    if d:IsA("TextLabel") and type(d.Text) == "string"
                       ...
                       and not seen[d.Text] then
                        seen[d.Text] = true
                        print("[VOIDHUB-DIALOG] (UNRELIABLE - may be a leftover from a previous session) "
...
local function syncCalibLabel()
    ...
    pcall(function()
        calibLabel:SetText(("%d studs/s  |  %d ok / %d failed  |  %d/%d to speed up")
    ...
    pcall(function()
        ...
        for _, r in ipairs(net:GetChildren()) do
            if r:IsA("RemoteEvent") then
                local nm = r.Name
                VoidHubTop[#VoidHubTop + 1] = r.OnClientEvent:Connect(function(...)
...
pcall(function()
    VoidHubTop.jump = UserInputService.JumpRequest:Connect(function()
    ...
    while stealing do
        ...
        if BX.riftOnly then
            ...
            if want and next(want) == nil then
                ...
                VoidHubNote("rift-covered", BX.riftAutoTrade
            ...
            if pick then
                ...
                elseif os.clock() - (BX.riftWaitAt or 0) > 8 then
                    ...
                    VoidHubNote("rift-wait", "Rift: waiting for " .. tostring(waitingFor), 120)
                end
                task.wait(1)
                continue
            end
        ...
        if selectedEggUid and (not target) and (not BX.followBest) then
            ...
            if BX.pickWasUser then
                ...
                VoidHubPickPrompt("Egg done - pick your next one")
            end
        elseif target then
        ...
        if not selectedEggUid and not BX.followBest then
            if not BX.saidPickOne then
                ...
                VoidHubPickPrompt("Pick an egg to steal")
            end
            task.wait(0.5)
            continue
        end
...
VoidHubTop.charAdded = LocalPlayer.CharacterAdded:Connect(function()
    task.wait(1)
...
task.spawn(function()
    ...
    for _ = 1, 40 do
        ...
        for _, root in ipairs({ (gethui and gethui()) or nil, game:GetService("CoreGui") }) do
            ...
            if ok and d and d:IsA("ImageLabel") and d.Parent and d.Parent.Name == "VoidHub" then
                icon = d
                break
            end
        end
        if icon then
            icon.Image = LOGO
            VoidHubTop.logoGuard = icon:GetPropertyChangedSignal("Image"):Connect(function()
...
if LocalPlayer.Name ~= LocalPlayer.DisplayName then
    profileName = profileName .. "  (@" .. LocalPlayer.Name .. ")"
else
    profileName = "@" .. LocalPlayer.Name
end
profileBox:AddLabel({ Text = profileName, DoesWrap = true })
profileBox:AddLabel({ Text = "User ID: " .. tostring(LocalPlayer.UserId), DoesWrap = true })

local gameBox = rawHome:AddGroupbox({ Side = "Right", Name = "Game" })
local gameNameLabel = gameBox:AddLabel({ Text = "Game: " .. tostring(game.Name), DoesWrap = true })
local gameTimeLabel = gameBox:AddLabel({ Text = "Game Time: 00:00:00", DoesWrap = true })
local homeOpenedAt = os.clock()
...
task.spawn(function()
    ...
    if ok and info and info.Name then
        pcall(function() gameNameLabel:SetText("Game: " .. tostring(info.Name)) end)
    end
end)

task.spawn(function()
    local __gen = (getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN
    while ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) == __gen do
        local elapsed = math.max(0, math.floor(os.clock() - homeOpenedAt))
        pcall(function()
            gameTimeLabel:SetText(("Game Time: %02d:%02d:%02d"):format(
...
__gbSteal:AddLabel({
    Text = "How it works\nPick an egg, turn on Auto Steal. The guard hitting you the first"
...
__gnSteal = __gnSteal + 1
__gbSteal = (__gnSteal % 2 == 1) and rawSteal:AddGroupbox({Side="Left", Name="Steal"}) or rawSteal:AddGroupbox({Side="Right", Name="Steal"})

eggDropdown = _mkProxy(__gbSteal:AddDropdown("TargetEgg",{
    Text = "Target Egg",
    Values = { "Loading..." },
Callback = UI.onDropdown,
    Searchable = true,
})); _reg("TargetEgg", eggDropdown)
...
function UI.applyOptions(options)
    ...
    if label == BX.lastDropdownLabel then return end
    UI.suppress = true
    local applied, err = pcall(d.SetValues, d, options)

    
    if not applied and tostring(err):find("capability") then
        task.wait()
        applied, err = pcall(d.SetValues, d, options)
        if applied then trace("dropdown: rebuilt after yielding (capability)") end
    end

    if not applied then
        trace("dropdown: :SetValues() threw: " .. tostring(err))
    end
...
function UI.showSelected()
    ...
    UI.suppress = true
    local ok, err = pcall(d.SetValue, d, label)
    UI.suppress = false
    if not ok then
        trace("dropdown: :SetValue() threw: " .. tostring(err))
...
X.el.autoSteal =_mkProxy(__gbSteal:AddToggle("AutoSteal",{
    Text = "Auto Steal",
Callback = function(value)
        
        
        if UI.refreshing then
            trace("autosteal: ignoring callback - not a click (value=" .. tostring(value) .. ")")
            return
        end

        local age = BX.autoStealSetAt and (os.clock() - BX.autoStealSetAt) or -1
        trace(("autosteal: callback value=%s stealing=%s %.2fs since the last one")
        ...
        if value then
            ...
            if type(BX.farmFiltering) == "function" and BX.farmFiltering() then
                BX.farmActive = true
                if type(BX.syncFollowBest) == "function" then BX.syncFollowBest() end
            end
            ...
            trace("autosteal: ON")
            local farmInfo = (BX.farmActive and " (farm filters active)") or ""
            toast("Auto Steal ON" .. farmInfo)
        else
            BX.holdGen = (BX.holdGen or 0) + 1
...
})); _reg("AutoSteal", X.el.autoSteal)
X.el.autoSteal:AddKeyPicker("AutoStealKey", {
    Text = "Auto Steal",
    Default = "F6",
    Mode = "Toggle",
...
;(function()
    local frames = 0
    VoidHubTop.liteFrames = RunService.Heartbeat:Connect(function() frames = frames + 1 end)

    local function goLite(fps)
        if liteMode then return end
        if not VoidHubAutoDPI then return end
        liteMode = true
        BX.lite = true   
    ...
    task.spawn(function()
        ...
        pcall(function() VoidHubTop.liteFrames:Disconnect() end)
    end)
end)()
...
function VoidHubEspScale(dist)
    return math.clamp(1.25 - (tonumber(dist) or 0) / 800, 0.6, 1.25)
end

VoidHubTop.espVis = RunService.RenderStepped:Connect(function()
...
__gnSteal = __gnSteal + 1
__gbSteal = (__gnSteal % 2 == 1) and rawSteal:AddGroupbox({Side="Left", Name="ESP"}) or rawSteal:AddGroupbox({Side="Right", Name="ESP"})

_reg("EggESP", __gbSteal:AddToggle("EggESP",{
    Text = "Egg ESP",
    Tooltip = "Shows every egg through walls",
Callback = function(v)
...
;(function()
    ...
    local scaleConn = RunService.RenderStepped:Connect(function()
        ...
        for _, c in pairs(cards) do
            if c.anchor and c.anchor.Parent then
                local s = VoidHubEspScale((c.anchor.Position - eye).Magnitude)
                if math.abs((c.lastScale or -1) - s) > 0.01 or c.lastH ~= c.baseH then
                    c.lastScale, c.lastH = s, c.baseH
                    c.scale.Scale = s
    ...
    task.spawn(function()
        local __gen = (getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN
        while true do
            if ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) ~= __gen then
    ...
    _reg("PlotESP", __gbSteal:AddToggle("PlotESP",{
        Text = "My Plot ESP",
        Tooltip = "The pet inside each egg on your plot, and when it hatches",
Callback = function(v)
...
;(function()
    ...
    local function makeTag(p)
        local th = VoidHubThemeCurrent or {}
    ...
    local lastFadeAt = 0
    VoidHubTop.mateFade = RunService.RenderStepped:Connect(function()
        local now = os.clock()
    ...
    VoidHubTop.mateLeave = Players.PlayerRemoving:Connect(dropTag)
...
__gnSteal = __gnSteal + 1
__gbSteal = (__gnSteal % 2 == 1) and rawSteal:AddGroupbox({Side="Left", Name="Plot"}) or rawSteal:AddGroupbox({Side="Right", Name="Plot"})
...
X.el.autoDoff =_mkProxy(__gbSteal:AddToggle("AutoDoff",{
    Text = "Stay off the treadmill",
    Tooltip = "Gets you off the treadmill",
Callback = function(v)
        BX.autoDoff = v and true or false
        toast("Treadmill guard " .. (BX.autoDoff and "ON" or "OFF"))
    end,
})); _reg("AutoDoff", X.el.autoDoff)
...
__gnSteal = __gnSteal + 1
__gbSteal = (__gnSteal % 2 == 1) and rawSteal:AddGroupbox({Side="Left", Name="Snipe & Recover"}) or rawSteal:AddGroupbox({Side="Right", Name="Snipe & Recover"})

__gbSteal:AddLabel({ Text = "Grab loose eggs and protect yourself during the run", DoesWrap = true })

BX.snipeActive = false
BX.godmodeActive = false
BX.autoPlaceActive = false

_reg("SnipeDroppedEgg", __gbSteal:AddToggle("SnipeDroppedEgg", {
    Text = "Snipe Dropped Egg",
    Tooltip = "Scans the field every 2s for loose eggs and grabs them via AskFieldEggCarry",
    Default = false,
    Callback = function(v)
        BX.snipeActive = v and true or false
        toast("Snipe Dropped Egg " .. (v and "ON" or "OFF"))
        trace("snipe: " .. tostring(v))
    end,
}))

_reg("AutoPlaceEgg", __gbSteal:AddToggle("AutoPlaceEgg", {
    Text = "Auto Place Egg",
    Tooltip = "Automatically places eggs from your satchel into nests via AskPlaceEgg every 5s",
    Default = false,
    Callback = function(v)
        BX.autoPlaceActive = v and true or false
        toast("Auto Place Egg " .. (v and "ON" or "OFF"))
        trace("autoplace: " .. tostring(v))
    end,
}))

_reg("Godmode", __gbSteal:AddToggle("Godmode", {
    Text = "Godmode",
    Tooltip = "Locks health to max every 0.1s so you cannot die",
    Default = false,
    Callback = function(v)
        BX.godmodeActive = v and true or false
        toast("Godmode " .. (v and "ON" or "OFF"))
        trace("godmode: " .. tostring(v))
    end,
}))
...
do
    ...
    task.spawn(function()
        while true do
            if ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) ~= __gen then return end
            task.wait(2)
            if BX.snipeActive then
                local rf = needRF("RF/EggWorld/AskFieldEggCarry")
                if rf then
                    for _, obj in ipairs(game:GetService("Workspace"):GetDescendants()) do
                        if obj:IsA("BasePart") and obj.Name:lower():find("egg") then
                            local ok, result = pcall(function()
                                return rf:InvokeServer(obj)
                            end)
                            if ok and result then
                                toast("Sniped a dropped egg!")
                                trace("snipe: grabbed " .. obj.Name)
                                break
                            end
                        end
                    end
    ...
    task.spawn(function()
        while true do
            if ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) ~= __gen then return end
            task.wait(5)
            if BX.autoPlaceActive then
                local rf = needRF("RF/EggWorld/AskPlaceEgg")
                if rf then
                    local ok, result = pcall(function()
                        return rf:InvokeServer()
                    end)
                    trace(("autoplace: AskPlaceEgg -> ok=%s result=%s"):format(tostring(ok), tostring(result)))
                    if ok and result then toast("Auto placed egg!") end
                end
            end
        end
    end)
...
__gbFarm:AddLabel({
    Text = "How it works\nPick what you want, then start Auto Steal HERE and it only takes"
        .. " those. Starting it on Main ignores these and takes everything.",
    DoesWrap = true
})

__gnFarm = __gnFarm + 1
__gbFarm = (__gnFarm % 2 == 1) and rawFarm:AddGroupbox({Side="Left", Name="Steal"}) or rawFarm:AddGroupbox({Side="Right", Name="Steal"})
...
do
    ...
    _reg("FarmAreas", __gbFarm:AddDropdown("FarmAreas",{
        Text = "Areas",
        Values = areaNames,
        Multi = true,
Callback = function(list)
    ...
    _reg("FarmRarities", __gbFarm:AddDropdown("FarmRarities",{
        Text = "Rarities",
        Values = rarityNames,
        Multi = true,
Callback = function(list)
    ...
    _reg("FarmTargetBy", __gbFarm:AddDropdown("FarmTargetBy",{
        Text = "Target By",
        Values = { "Income", "Weight" },
Callback = function(v)
    ...
    X.el = X.el or {}
    X.el.farmAuto =_mkProxy(__gbFarm:AddToggle("FarmAutoSteal",{
        Text = "Auto Steal",
        Tooltip = "Same as Main, but only takes what you picked above",
Callback = function(v)
    ...
    })); _reg("FarmAutoSteal", X.el.farmAuto)
...
__gnFarm = __gnFarm + 1
__gbFarm = (__gnFarm % 2 == 1) and rawFarm:AddGroupbox({Side="Left", Name="AFK"}) or rawFarm:AddGroupbox({Side="Right", Name="AFK"})
...
do
    ...
    X.el = X.el or {}
    X.el.treadmill =_mkProxy(__gbFarm:AddToggle("StayTreadmill",{
        Text = "Stay On Treadmill",
        Tooltip = "Stands on your treadmill and keeps you there",
Callback = function(v)
            v = v and true or false
            ...
            if v and stealing then
                ...
                pcall(function()
                    ...
                    if el then el:SetValue(false) end
                end)
                return
            end
    ...
    })); _reg("StayTreadmill", X.el.treadmill)
...
do
    ...
    local function doHatch()
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if not net then return false, "networking not found" end
        
        local rf = net:FindFirstChild("RF/Haul/AskHatchAll")
        if rf then
            local ok, count = pcall(function() return rf:InvokeServer() end)
            if ok and type(count) == "number" and count > 0 then
                trace(("autohatch: hatched %d egg(s)"):format(count))
                toast("Hatched " .. count .. " egg" .. (count ~= 1 and "s" or "") .. "!")
                return true, count
            end
        end
        ...
        local hatched = 0
        pcall(function()
            local gui = LocalPlayer.PlayerGui:FindFirstChild("GrowingEggs", true)
            if not gui then return end
            for _, frame in ipairs(gui:GetDescendants()) do
                if frame:IsA("TextButton") and
                   (frame.Text:lower():find("hatch") or frame.Text:lower():find("collect")) then
                    pcall(function()
                        local trigger = Instance.new("MouseButton1Click")
                        frame:FireRemoteEvent(trigger)
                    end)
                    local rf2 = net:FindFirstChild("RF/GrowingEgg/AskHatch")
                    if rf2 then
                        local ok2, _ = pcall(function()
                            local uid = frame.Parent and frame.Parent:GetAttribute("EggUID")
                            if uid then rf2:InvokeServer(uid) end
                        end)
                        if ok2 then hatched = hatched + 1 end
                    end
                end
            end
        end)
        if hatched > 0 then toast("Hatched " .. hatched .. " egg(s)") end
        return hatched > 0, hatched
    end
    ...
    __gnFarm = __gnFarm + 1
    __gbFarm = (__gnFarm % 2 == 1) and rawFarm:AddGroupbox({Side="Left", Name="Auto Hatch"}) or rawFarm:AddGroupbox({Side="Right", Name="Auto Hatch"})

    __gbFarm:AddLabel({ Text = "Hatches ready eggs on your plot every 10s", DoesWrap = true })

    _reg("AutoHatch", __gbFarm:AddToggle("AutoHatch", {
        Text = "Auto Hatch Eggs",
        Default = false,
        Tooltip = "Automatically hatches eggs when they're ready",
        Callback = function(v)
            hatchActive = v and true or false
            if v then pcall(doHatch) end
            toast("Auto Hatch " .. (v and "ON" or "OFF"))
            trace("autohatch: " .. tostring(v))
        end,
    }))

    __gbFarm:AddButton({ Text = "Hatch Now", Func = function()
        local ok, n = pcall(doHatch)
        if not ok then toast("Hatch failed") end
    end })
end
...
do
    ...
    local function doAwayCollect()
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if not net then return false end
        
        local rfCheck = net:FindFirstChild("RF/AwayEarnings/PendingCheck")
        if rfCheck then
            local ok, pending = pcall(function() return rfCheck:InvokeServer() end)
            if ok and not pending then return false end
        end
        local rf = net:FindFirstChild("RF/AwayEarnings/AskCollect")
        if not rf then
            
            rf = RS:FindFirstChild("RF/AwayEarnings/AskCollect", true)
        end
        if not rf then return false end
        local ok, result = pcall(function() return rf:InvokeServer() end)
        trace(("awayearnings: AskCollect -> ok=%s result=%s"):format(tostring(ok), tostring(result)))
        if ok and result then
            toast("Away earnings collected!")
            return true
        end
        return false
    end
    ...
    __gnFarm = __gnFarm + 1
    __gbFarm = (__gnFarm % 2 == 1) and rawFarm:AddGroupbox({Side="Left", Name="Away Earnings"}) or rawFarm:AddGroupbox({Side="Right", Name="Away Earnings"})

    __gbFarm:AddLabel({ Text = "Collects your offline income every 60s", DoesWrap = true })

    _reg("AutoAwayEarnings", __gbFarm:AddToggle("AutoAwayEarnings", {
        Text = "Auto Collect Away Earnings",
        Default = false,
        Tooltip = "Automatically collects away earnings every minute",
        Callback = function(v)
            awayActive = v and true or false
            if v then pcall(doAwayCollect) end
            toast("Away Earnings auto-collect " .. (v and "ON" or "OFF"))
            trace("awayearnings: " .. tostring(v))
        end,
    }))

    __gbFarm:AddButton({ Text = "Collect Now", Func = function()
        local ok = pcall(doAwayCollect)
        if not ok then toast("Collect failed") end
    end })
end

do
    local rarityList = { "Garden", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Secret", "Impossible", "Event", "Divine" }
    BX.minRarityRank = 0
    local rarityRank = {}
    for i, v in ipairs(rarityList) do rarityRank[v] = i end

    BX.autoSellActive = false

    __gnFarm = __gnFarm + 1
    __gbFarm = (__gnFarm % 2 == 1) and rawFarm:AddGroupbox({Side="Left", Name="Rarity Filter"}) or rawFarm:AddGroupbox({Side="Right", Name="Rarity Filter"})

    __gbFarm:AddLabel({ Text = "Keep Min Rarity — Auto Steal skips eggs ranked below your pick", DoesWrap = true })

    _reg("MinRarity", __gbFarm:AddDropdown("MinRarity", {
        Text = "Keep Min Rarity",
        Tooltip = "Eggs below this rarity are skipped by Auto Steal",
        Values = rarityList,
        Default = 1,
        Callback = function(v)
            BX.minRarityRank = rarityRank[v] or 0
            toast("Min rarity: " .. tostring(v))
            trace("minrarity: " .. tostring(v) .. " rank=" .. tostring(BX.minRarityRank))
        end,
    }))

    _reg("AutoSellEgg", __gbFarm:AddToggle("AutoSellEgg", {
        Text = "Auto Sell Egg",
        Tooltip = "Pushes auto-sell config to the server via WriteAutoSell",
        Default = false,
        Callback = function(v)
            BX.autoSellActive = v and true or false
            local rf = needRF("RF/Haul/WriteAutoSell")
            if rf then
                local ok, result = pcall(function()
                    return rf:InvokeServer(v and true or false)
                end)
                trace(("autosell: WriteAutoSell -> ok=%s result=%s"):format(tostring(ok), tostring(result)))
            end
            toast("Auto Sell Egg " .. (v and "ON" or "OFF"))
            trace("autosell: " .. tostring(v))
        end,
    }))

    __gbFarm:AddButton({ Text = "Sell Full Satchel Now", Func = function()
        local rf = needRF("RF/Haul/OfferFullSatchelSale")
        if not rf then toast("Remote not found") return end
        local ok, result = pcall(function() return rf:InvokeServer() end)
        trace(("satchelsale: -> ok=%s result=%s"):format(tostring(ok), tostring(result)))
        toast(ok and result and "Satchel sold!" or "Sale refused or failed")
    end })
end
...
__gnEvent = __gnEvent + 1
__gbEvent = (__gnEvent % 2 == 1) and rawEvent:AddGroupbox({Side="Left", Name="Boss"}) or rawEvent:AddGroupbox({Side="Right", Name="Boss"})

X.boss.line = __gbEvent:AddLabel({ Text = "Abyss Overlord — Reading...", DoesWrap = true })

__gbEvent:AddButton({
    Text = "Enter the boss world",
...
_reg("AutoBoss", __gbEvent:AddToggle("AutoBoss",{
    Text = "Auto enter",
    Tooltip = "Joins as soon as it opens",
Callback = function(v)
...
_reg("AutoBossFight", __gbEvent:AddToggle("AutoBossFight",{
    Text = "Auto fight",
    Tooltip = "Breaks the crystals, then the boss",
Callback = function(v)
...
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
            ...
            if not snap then
                X.boss.line:SetText("Could not read the boss event")
                return
            end
            local now = workspace:GetServerTimeNow()
...
function X.riftForgetPick()
    ...
    pcall(function()
        ...
        X.rift.pick:SetValues(opts)
        X.rift.pick:SetValue(opts[1])
...
do
    BX.bloomActive = false

    local function doBloomeryPetal()
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if not net then return false end
        local rf = net:FindFirstChild("RF/Bloomery/AskGatherPetal")
        if not rf then return false end
        local ok, result = pcall(function() return rf:InvokeServer() end)
        trace(("bloomery: AskGatherPetal -> ok=%s result=%s"):format(tostring(ok), tostring(result)))
        return ok and result
    end

    local function doBloomeryMutate()
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if not net then return false end
        local rf = net:FindFirstChild("RF/Bloomery/AskMutate")
        if not rf then return false end
        local ok, result = pcall(function() return rf:InvokeServer() end)
        trace(("bloomery: AskMutate -> ok=%s result=%s"):format(tostring(ok), tostring(result)))
        if ok and result then toast("Bloomery mutation applied!") end
        return ok and result
    end
    ...
    task.spawn(function()
        ...
        while true do
            if ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) ~= __gen then return end
            task.wait(8)
            if BX.bloomActive then
                pcall(doBloomeryPetal)
                task.wait(1)
                pcall(doBloomeryMutate)
            end
        end
    end)

    __gnEvent = __gnEvent + 1
    __gbEvent = (__gnEvent % 2 == 1) and rawEvent:AddGroupbox({Side="Left", Name="Bloomery"}) or rawEvent:AddGroupbox({Side="Right", Name="Bloomery"})

    __gbEvent:AddLabel({ Text = "Gathers petals and applies mutations every 8s", DoesWrap = true })

    _reg("AutoBloomery", __gbEvent:AddToggle("AutoBloomery", {
        Text = "Auto Bloomery",
        Default = false,
        Tooltip = "Auto-gathers petals and mutates eggs in the Bloomery",
        Callback = function(v)
            BX.bloomActive = v and true or false
            toast("Bloomery auto " .. (v and "ON" or "OFF"))
            trace("bloomery: " .. tostring(v))
        end,
    }))

    __gbEvent:AddButton({ Text = "Gather Petal Now", Func = function() pcall(doBloomeryPetal) end })
    __gbEvent:AddButton({ Text = "Mutate Now",       Func = function() pcall(doBloomeryMutate) end })
end


do
    __gnEvent = __gnEvent + 1
    __gbEvent = (__gnEvent % 2 == 1) and rawEvent:AddGroupbox({Side="Left", Name="Boss Shop"}) or rawEvent:AddGroupbox({Side="Right", Name="Boss Shop"})

    __gbEvent:AddLabel({ Text = "Spend Boss Tokens from RF/BossMastery/AskBuyShopItem", DoesWrap = true })
    ...
    local selectedBossItem = BOSS_ITEMS[1]
    _reg("BossShopItem", __gbEvent:AddDropdown("BossShopItem", {
        Text = "Shop Item",
        Values = BOSS_ITEMS,
        Default = 1,
        Searchable = false,
    ...
    __gbEvent:AddButton({ Text = "Buy Selected Item", Func = function()
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if not net then toast("Networking not found") return end
        local rf = net:FindFirstChild("RF/BossMastery/AskBuyShopItem")
        if not rf then toast("Boss shop remote not found") return end
        local ok, result = pcall(function() return rf:InvokeServer(selectedBossItem) end)
        trace(("bossshop: AskBuyShopItem(%s) -> ok=%s result=%s"):format(
            tostring(selectedBossItem), tostring(ok), tostring(result)))
        if ok and result then
            toast("Bought: " .. tostring(selectedBossItem))
        else
            toast(tostring(result or "Not enough tokens"))
        end
    end })

    __gbEvent:AddButton({ Text = "Claim Mastery Milestones", Func = function()
        local n = BX.claimBossMilestones and BX.claimBossMilestones() or 0
        toast(n > 0 and ("Claimed " .. n .. " milestone(s)") or "Nothing to claim yet")
    end })
end


__gnEvent = __gnEvent + 1
__gbEvent = (__gnEvent % 2 == 1) and rawEvent:AddGroupbox({Side="Left", Name="Rift"}) or rawEvent:AddGroupbox({Side="Right", Name="Rift"})

X.rift.line = __gbEvent:AddLabel({ Text = "Rift — reading...", DoesWrap = true })

X.rift.pick = _mkProxy(__gbEvent:AddDropdown("RiftPet",{
    Text = "Rift pet",
    Values = { X.rift.none },
Callback = function(v)
        ...
        if BX.riftPet then
            ...
            VoidHubNote("rift-pick", "Rift pet: " .. X.petName(id), 3)
        end
    end,
    Searchable = true,
})); _reg("RiftPet", X.rift.pick)
...
__gbEvent:AddButton({
    ...
    Func = function()
        ...
        VoidHubNote("rift-refresh", (#out > 0) and ("Rift: " .. table.concat(X.petNames(out), ", ")) or "Rift: none out", 2)
    end,
})

_reg("RiftAuto", __gbEvent:AddToggle("RiftAuto",{
    Text = "Auto steal rift pets",
    Tooltip = "Steals only the rift pets",
Callback = function(v)
        BX.riftOnly = v and true or false
        trace("rift: auto " .. (BX.riftOnly and "ON" or "OFF"))
        VoidHubNote("rift-auto", "Rift auto " .. (BX.riftOnly and "ON" or "OFF"), 2)
        
        local el = X.el and X.el.autoSteal
        if el then
            pcall(function() el:SetValue(BX.riftOnly) end)
            return
        end
...
_reg("RiftAutoTrade", __gbEvent:AddToggle("RiftAutoTrade",{
    Text = "Auto trade-in",
    Tooltip = "Puts your 3 rift pets in the Rift when you have them (lightest first, never equipped)",
Callback = function(v)
        BX.riftAutoTrade = v and true or false
        X.riftTradeAt = 0
        trace("rift: auto trade-in " .. (BX.riftAutoTrade and "ON" or "OFF"))
        VoidHubNote("rift-trade-toggle", "Auto trade-in " .. (BX.riftAutoTrade and "ON" or "OFF"), 2)
    end,
}))
...
function X.riftPaint()
    ...
    local function set(body, title)
        local text = title and (tostring(title) .. " — " .. tostring(body)) or tostring(body)
        if text ~= X.riftLastText then
            X.riftLastText = text
            pcall(function() X.rift.line:SetText(text) end)
        end
    end
...
task.spawn(function()
    ...
    while true do
        ...
        pcall(function()
               ...
               and os.clock() - (X.riftTradeAt or 0) > 5 then
                ...
                task.spawn(function()
                    ...
                    if r == "traded" then
                        ...
                        VoidHubNote("rift-traded", "Rift: traded in - Rift Egg added to your eggs", 5)
                    elseif r == "revealed" then
                        trace("rift: finished a pending reveal")
                    elseif r then
                        trace("rift: trade-in " .. tostring(r))
                        VoidHubNote("rift-trade-fail", "Rift trade-in " .. tostring(r), 60)
                    end
                end)
            end
            ...
            if ready and not lastReady then VoidHubNote("rift-ready", "Rift: all pets ready", 120) end
            lastReady = ready

            if lastBanner and st.BannerId ~= lastBanner then
                VoidHubNote("rift-banner", "Rift: new banner " .. tostring(st.BannerDisplayName or st.BannerId), 30)
            end
            ...
            if pc and pt and pc >= pt - 5 and lastPity and pc > lastPity then
                VoidHubNote("rift-pity", ("Rift: pity %d/%d"):format(pc, pt), 60)
            end
            lastPity = pc
        end)
    end
...
pcall(function()
    ...
    if ev then
        VoidHubTop.bannerRotated = ev.OnClientEvent:Connect(function()
            X.riftSnapAt = 0
            pcall(X.riftPaint)
        end)
    end
...
do
    local ZONE_SPAWNS = {
        ["Abyss Ocean"]   = Vector3.new(-1400,  70,  1400),
        ["Cherry Blossom"]= Vector3.new(-1800, 100,   200),
        ["Cosmic"]        = Vector3.new(  200, 400,   200),
        ["Desert"]        = Vector3.new(  800,  70,   -50),
        ["Forest"]        = Vector3.new( -400,  70,  -600),
        ["Jungle"]        = Vector3.new( -200,  70,  1000),
        ["Lake"]          = Vector3.new(  200,  70,   200),
        ["Prehistoric"]   = Vector3.new(-1000,  70,  -600),
        ["Snow"]          = Vector3.new( 1200, 120,  -400),
        ["Titan Temple"]  = Vector3.new(  200, 200,  -1600),
        ["Volcano"]       = Vector3.new( 1800, 200,  -200),
    }

    local function tryZoneTeleport(zoneName)
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if net then
            local rf = net:FindFirstChild("RF/Zones/AskTeleport")
                   or net:FindFirstChild("RF/Area/AskTeleport")
            if rf then
                local ok, result = pcall(function() return rf:InvokeServer(zoneName) end)
                if ok and result then
                    toast("Teleported to " .. zoneName)
                    trace(("zone: RF teleport to %s ok"):format(zoneName))
                    return
                end
            end
        end
        local spawn = ZONE_SPAWNS[zoneName]
        if spawn then
            local hrp = getHRP()
            if hrp then
                hrp.CFrame = CFrame.new(spawn)
                toast("Jumped to " .. zoneName)
                trace(("zone: CFrame jump to %s"):format(zoneName))
                return
            end
        end
        toast("Could not teleport to " .. zoneName)
    end

    __gbMisc = (__gnMisc % 2 == 1) and rawMisc:AddGroupbox({Side="Left", Name="Zone Teleporter"}) or rawMisc:AddGroupbox({Side="Right", Name="Zone Teleporter"})

    local zoneNames = {}
    for k in pairs(ZONE_SPAWNS) do zoneNames[#zoneNames + 1] = k end
    table.sort(zoneNames)

    local selectedZone = zoneNames[1] or "Forest"
    _reg("ZonePick", __gbMisc:AddDropdown("ZonePick", {
        Text = "Zone",
        Values = zoneNames,
        Default = 1,
        Searchable = true,
    ...
    __gbMisc:AddButton({ Text = "Teleport to Zone", Func = function()
        if selectedZone then pcall(tryZoneTeleport, selectedZone) end
    end })

    __gbMisc:AddButton({ Text = "Return to Plot", Func = function()
        local net = RS.Packages and RS.Packages:FindFirstChild("Networking")
        if net then
            local rf = net:FindFirstChild("RF/Plot/AskTeleport")
                   or net:FindFirstChild("RF/Haul/GoHome")
            if rf then
                local ok, _ = pcall(function() return rf:InvokeServer() end)
                if ok then toast("Going home") return end
            end
        end
        local spot = BX.treadmillSpot and BX.treadmillSpot()
        local hrp = getHRP()
        if spot and hrp then
            hrp.CFrame = CFrame.new(spot + Vector3.new(0, 5, 0))
            toast("Back at your plot")
        else
            toast("Could not find your plot")
        end
    end })
end

__gnMisc = __gnMisc + 1
__gbMisc = (__gnMisc % 2 == 1) and rawMisc:AddGroupbox({Side="Left", Name="Look"}) or rawMisc:AddGroupbox({Side="Right", Name="Look"})
...
do
    ...
    local function pal(bgTop, bgBottom, tab, element, field, accent, accentLight)
        ...
        VoidHubThemeRaw = VoidHubThemeRaw or {}
        VoidHubThemeRaw[t] = { bgTop = bgTop, bgBottom = bgBottom, element = element,
                             accent = accent, accentLight = accentLight }
        return t
    end

    
    function VoidHubApplyThemeExtras(pick)
        VoidHubThemeCurrent = (pick and VoidHubThemeRaw and VoidHubThemeRaw[pick]) or {
        ...
        if type(VoidHubOnTheme) == "function" then pcall(VoidHubOnTheme, VoidHubThemeCurrent) end
    end
    ...
    local themeDrop
    local originalDefaultTheme = ThemeManager and ThemeManager.BuiltInThemes
        and ThemeManager.BuiltInThemes.Default

    local function applyObsidianTheme(pick)
        if not ThemeManager then return false end
        local theme = pick or originalDefaultTheme
        if theme then ThemeManager:SetDefaultTheme(theme) end
    ...
    function X.setTheme(name, quiet)
        ...
        pcall(applyObsidianTheme, pick)
        VoidHubApplyThemeExtras(pick)
        if themeDrop then
            pcall(function() themeDrop:SetValue(tostring(name)) end)
        end
        if not quiet then trace("theme: " .. tostring(name)) end
        return true
    end

    themeDrop = _mkProxy(__gbMisc:AddDropdown("HubTheme",{
        Text = "Theme",
        Values = names,
Callback = function(v)
            ...
            local applied = false
            local ok, result = pcall(applyObsidianTheme, pick)
    ...
    Searchable = true,
})); _reg("HubTheme", themeDrop)

    
    task.defer(function()
        pcall(function()
            applyObsidianTheme(THEMES.Mono)
            VoidHubApplyThemeExtras(THEMES.Mono)
            BX.theme = "Mono"
            if themeDrop then themeDrop:SetValue("Mono") end
...
do
    local function applyBackground(id)
        ...
        if id == "" then
            local ok, err = pcall(function() Window:SetBackgroundImage("") end)
            return ok, ok and "cleared" or tostring(err)
        end
    ...
    X.setBackground = function(id)
        return applyBackground(id)
    end

    X.el = X.el or {}
    X.el.bgInput = _mkProxy(__gbMisc:AddInput("HubBackground",{
        Text = "Background Image",
        Tooltip = "Paste a Roblox image id - empty clears it",
        Placeholder = "0000000000",
Callback = function(v)
            BX.bgImage = tostring(v or "")
            
            
            local ok, why = X.setBackground(v)
            trace("background: " .. tostring(why))
            toast(ok and ("Background " .. why) or ("Background failed - " .. why))
        end,
    })); _reg("HubBackground", X.el.bgInput)
...
do
    ...
    _reg("FpsBoost", __gbMisc:AddToggle("FpsBoost",{
        Text = "FPS Boost",
        Tooltip = "Turns off effects and shadows - nothing is deleted",
...
;(function()
    ...
    local STROKE_T = 0.45
    local POS_FILE = "VoidHub_stats_pos.json"

    local ICON_DIR = "VoidHub/icons"
    ...
    task.spawn(function()
        ...
        pcall(function()
            if not isfolder("VoidHub") then makefolder("VoidHub") end
    ...
    takeTheme(VoidHubThemeCurrent)

    function VoidHubOnTheme(raw)
        takeTheme(raw)
    ...
    local function build()
        local parent = (gethui and gethui()) or game:GetService("CoreGui")
        local old = parent:FindFirstChild("VoidHubStats")
        if old then old:Destroy() end

        gui = mk("ScreenGui", {
            Name = "VoidHubStats", DisplayOrder = 999990, ResetOnSpawn = false,
    ...
    X.el = X.el or {}
    X.el.statsHud =_mkProxy(__gbMisc:AddToggle("StatsHud",{
        Text = "Stats Overlay",
        Tooltip = "Session time, FPS and ping - drag to move, double-click to reset",
Callback = function(v) show(v and true or false) end,
    })); _reg("StatsHud", X.el.statsHud)
...
__gnMisc = __gnMisc + 1
__gbMisc = (__gnMisc % 2 == 1) and rawMisc:AddGroupbox({Side="Left", Name="Performance"}) or rawMisc:AddGroupbox({Side="Right", Name="Performance"})

_reg("AutoDPI", __gbMisc:AddToggle("AutoDPI",{
    Text = "Auto DPI",
    Tooltip = "Automatically reduces visual effects when FPS drops below 25. Turn off to keep full detail always.",
Default = true,
    Callback = function(v)
        VoidHubAutoDPI = v and true or false
        if not v and liteMode then
            liteMode = false
            BX.lite = false
            K.ESP_MAX = 80
            K.ESP_REFRESH = 0.4
            K.ESP_VIS_HZ = 20
            K.ESP_BUILD_PER_PASS = 12
        end
        toast(v and "Auto DPI on — adapts to your device" or "Auto DPI off — full detail always")
        trace("autoDPI: " .. tostring(v))
    end,
}))

_reg("WalkSpeed", __gbMisc:AddSlider("WalkSpeed", {
    Text = "Walkspeed",
    Tooltip = "Override your character's WalkSpeed. 16 is the game default.",
    Min = 16,
    Max = 300,
    Default = 16,
    Rounding = 0,
    Callback = function(v)
        ...
        trace("walkspeed: " .. tostring(v))
    end,
}))

__gnMisc = __gnMisc + 1
__gbMisc = (__gnMisc % 2 == 1) and rawMisc:AddGroupbox({Side="Left", Name="Servers"}) or rawMisc:AddGroupbox({Side="Right", Name="Servers"})
...
_reg("JoinJobId", __gbMisc:AddInput("JoinJobId",{
    Text = "Join Job ID",
    Tooltip = "Paste an ID to join that server",
...
function X.setWidget(el, value)
    ...
    if type(el.SetValue) ~= "function" then return false end
    return pcall(el.SetValue, el, value)
end


function X.label(gb, text)
    if not gb then return nil end
    local ok, el = pcall(function()
        return gb:AddLabel({ Text = text, DoesWrap = true })
    end)
...
function X.setLabel(el, text)
    if not el then return end
    pcall(function()
        if type(el.SetText) == "function" then el:SetText(text)
        else el.Text = text end
    end)
end
...
task.spawn(function()
    ...
    while ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) == __gen do
        ...
        if not UI.refreshing then
            ...
            if not ok then
                UI.refreshing = false
                trace("auto refresh: " .. tostring(err))
            end
        end
    end
end)
...
task.spawn(function()
    ...
    while true do
        task.wait(1)
        local env = (type(getgenv) == "function" and getgenv()) or _G
        if env.__VOIDHUB_GEN ~= __gen then return end   
        local closed = false
        ...
        if closed then
            ...
            for _, holder in ipairs({ BX, VoidHubTop }) do
                for _, v in pairs(holder) do
                    if typeof(v) == "RBXScriptConnection" then
                        pcall(function() v:Disconnect() end)
                    end
...
webhookTab:AddToggle("WebhookEnabled", {
    Text = "Enable Webhook",
    Default = false,
    Callback = function(value) Webhook.Enabled = value and true or false end,
})

webhookTab:AddToggle("WebhookAllNotices", {
    Text = "Send All Notices",
    Default = false,
    Callback = function(value) Webhook.AllNotices = value and true or false end,
})
...
webhookTab:AddButton({
    ...
    Func = function()
        Window:AddDialog("WebhookTestDialog", {
            ...
            FooterButtons = {
                Cancel = { Title = "Cancel", Variant = "Ghost", Order = 1, Callback = function() end },
                Confirm = {
                    Title = "Send Test",
                    Variant = "Primary",
                    Order = 2,
                    Callback = function()
                        task.spawn(function()
                            local ok, why = Webhook:Send("Webhook connected", "VoidHub webhook test completed.", true)
                            toast(ok and "Webhook test sent" or ("Webhook failed: " .. tostring(why)))
                        end)
                    end,
                },
            },
...
local sessionOverlay
local sessionOverlayVisible = false
interfaceTab:AddToggle("ObsidianSessionOverlay", {
    Text = "Session Overlay",
    Default = false,
    Callback = function(value)
        sessionOverlayVisible = value and true or false
        if value and not sessionOverlay then
            sessionOverlay = Library:AddDraggableLabel("VoidHub | " .. LocalPlayer.Name, iconUrl("activity"), "Left")
        elseif sessionOverlay then
            sessionOverlay:SetVisible(value)
        end
    end,
})
...
task.spawn(function()
    local __gen = (getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN
    while ((getgenv and getgenv().__VOIDHUB_GEN) or _G.__VOIDHUB_GEN) == __gen do
        if sessionOverlay and sessionOverlayVisible then
            local elapsed = math.max(0, math.floor(os.clock() - homeOpenedAt))
            pcall(function()
                sessionOverlay:SetText(("VoidHub | %s | %02d:%02d:%02d"):format(
                    LocalPlayer.Name, math.floor(elapsed / 3600),
                    math.floor((elapsed % 3600) / 60), elapsed % 60))
            end)
        end
        task.wait(1)
    end
...
interfaceTab:AddToggle("ObsidianFloatingButton", {
    Text = "Floating Hub Button",
    Default = false,
    Callback = function(value)
        if value and not floatingButton then
            floatingButton = Library:AddDraggableButton("VoidHub", function() Window:Toggle() end, false, iconUrl("menu"), "Left")
        elseif not value and floatingButton then
...
interfaceTab:AddToggle("ObsidianKeybindMenu", {
    Text = "Keybind Menu",
    Default = false,
    Callback = function(value)
        if Library.KeybindFrame then Library.KeybindFrame.Visible = value and true or false end
    end,
})

interfaceTab:AddButton({ Text = "Pop Out Advanced Panel", Func = function() advancedBox:TogglePoppedOut() end })

Library:OnUnload(function()
    Webhook.Enabled = false
    if sessionOverlay then pcall(function() sessionOverlay:Destroy() end) end
    if floatingButton then pcall(function() floatingButton:Destroy() end) end
end)
...
if SaveManager then
    ...
    unavailable:AddLabel({ Text = "SaveManager could not be loaded on this executor.", DoesWrap = true })
end
...
trace("ui: Obsidian managers built")
...
;(function()
    ...
    VoidHubTop.pageEnter = layout.PageEnter:Connect(function(page)
        if liteMode then return end
        pcall(function()
            local sc = page:FindFirstChild("VoidHubPageScale") or Instance.new("UIScale")
            sc.Name = "VoidHubPageScale"
            sc.Parent = page
            sc.Scale = 0.975
...
pcall(function()
    ...
    if act and type(act.callback) == "function" then
        act.callback = function() Window:Toggle() end
...
pcall(function()
    local L, now = VoidHubLoadT, os.clock()
...
if false then task.spawn(function()
    ...
    local gui = new("ScreenGui", {
        Name = "VoidHubWelcome", DisplayOrder = 999999, IgnoreGuiInset = true,
        ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling,
    }, parent)
