...
local M = workspace:FindFirstChild("Enemies");
...
if not M then
	task.spawn(function()
		M = workspace:WaitForChild("Enemies", 60);
	end);
end;
Marines = function()
...
BringEnemy = function()
		...
		BFBringNextAt = now + .05;
		if now >= BFRadiusNextAt then
			BFRadiusNextAt = now + .5;
		...
		for _, R in pairs(M:GetChildren()) do
			...
			if humanoid and primary and humanoid.Health > 0 then
				if (primary.Position - PosMon).Magnitude <= 500 then
...
UpdStFruit = function()
		...
		for _, container in ipairs(containers) do
			for _, tool in ipairs(container:GetChildren()) do
				if BFIsFruitTool(tool) and not seen[tool] then
					seen[tool] = true;
...
local function inventorySnapshot()
	local now = os.clock();
	if now - inventoryCache.UpdatedAt < 1 then
...
BFAcceptQuest = function(questName, questTier, npcName)
		...
		task.spawn(function()
			...
			if wrapper then
				pcall(wrapper.InvokeServer, wrapper, questName, questTier);
			end;
		end);
...
BFRaidIslandNames = { "Island 5", "Island 4", "Island 3", "Island 2", "Island 1" };
...
_tp = function(I, forceTween)
		...
		local cappedSpeed = math.min(speed, 250);
		local tween = w:Create(root, TweenInfo.new(distance / cappedSpeed, Enum.EasingStyle.Linear), { CFrame = I });
		BFMove.Tween = tween;
		BFMove.Target = I;
		BFMove.Root = root;
		...
		tween:Play();
		-- Tween security kick guard: if server snaps us >80 studs backward, re-issue tween.
		task.spawn(function()
			local guardTween = tween;
			local lastPos = root.Position;
			while guardTween.PlaybackState == Enum.PlaybackState.Playing do
				task.wait(.1);
				if not root or not root.Parent then break; end;
				local nowPos = root.Position;
				if (nowPos - lastPos).Magnitude > 80 and BFMove.Tween == guardTween then
					guardTween:Cancel();
					_tp(I, forceTween);
					break;
				end;
...
function CheckQuest()
	...
	if World1 then
		...
		elseif MyLevel >= 250 and MyLevel <= 274 then
			...
			CFrameMon = CFrame.new(-1820.21484375, 10, -2740.6650390625);
...
UI.RootTabs = {
	...
	Priority = UI.Window:AddTab("Priority", "list-ordered", "Choose what the script runs first"),
...
UI.Groups = {
	{ Root = "Farm", Title = "Leveling", Side = "Left", Icon = "trending-up", Sections = {
		...
		{ Key = "BF/SmartFarm", Name = "Better Farm", Icon = "rocket", Tip = "Smarter level progression from live game data" },
		{ Key = "BF/MoneyFarm", Name = "Money Farm", Icon = "coins", Tip = "Beli from chests, bosses and quests at once" },
	...
	{ Root = "Farm", Title = "Island Secrets", Side = "Right", Icon = "star", Sections = {
		{ Key = "Island Secrets", Name = "Sea 1 Secrets", Icon = "map-pin", Tip = "Automate Sea 1 island secrets to earn stars and push past level 2800" },
	} },
...
UI.AutoFarmStep = function()
	...
	if not CheckHasQuest(NameMon) then
		...
		-- Buddha transforms enlarge the character making tight tolerances miss the NPC.
		-- Detect Buddha fruit equipped and use wider tolerance + precision notween.
		local isBuddha = false;
		do
			local df = (d:FindFirstChild("Data") or {}); df = df and df:FindFirstChild("DevilFruit");
			local dfName = df and tostring(df.Value or "") or "";
			isBuddha = dfName:lower():find("buddha") ~= nil;
			if not isBuddha then
				local char = d.Character;
				local tool = char and char:FindFirstChildOfClass("Tool");
				if tool then
					isBuddha = tool.Name:lower():find("buddha") ~= nil;
				end;
			end;
		end;
		local questTol = isBuddha and 8 or 4;
		-- Try up to 3 times to reach quest giver (handles NPC respawn / despawn)
		local reached = false;
		for attempt = 1, 3 do
			if isBuddha then
				notween(CFrameQuest * CFrame.new(0, 0, questTol));
				task.wait(.3);
			end;
			if BFMoveNear(CFrameQuest, questTol + 2) then
				reached = true;
				break;
			end;
			task.wait(.4);
		...
		if now >= UI.AutoFarmNextQuestAt then
			UI.AutoFarmNextQuestAt = now + 8;
			-- Brief wait for NPC model to stream in after arriving at quest giver position
			task.wait(0.6);
...
UI.SmartFarm = {
	...
	SpawnCache = {},
	NpcCache = {},
	CacheFile = "LuminHub/BloxFruits/SmartFarmNPCs.json",
	Status = "idle",
	Choice = nil,
	Stats = { Kills = 0, StartExp = 0, StartLevel = 0, StartClock = 0, LastExp = 0, LastLevel = 0 },
};
do
	local S = UI.SmartFarm;
	local BOSS_TAG = "%[Boss%]";

	local function DataNumber(name)
		return tonumber(BFDataValue(name)) or 0;
	end;

	function S.BuildLadder(force)
		local now = os.clock();
		if not force and #S.Ladder > 0 and now < S.LadderBuiltAt + 60 then
			return S.Ladder;
		end;
		local ok, quests = pcall(require, game:GetService("ReplicatedStorage"):WaitForChild("Quests", 5));
		...
		for questId, tiers in pairs(quests) do
			if type(tiers) == "table" then
				for tier, entry in ipairs(tiers) do
					if type(entry) == "table" and type(entry.Task) == "table" then
						local mobName, kills, distinct = nil, 0, 0;
						...
						if mobName and kills > 0 and exp > 0 then
							table.insert(ladder, {
								Id = questId,
								Tier = tier,
								LevelReq = tonumber(entry.LevelReq) or 0,
								Title = tostring(entry.Name or mobName),
								Mob = mobName,
								Kills = kills,
								Distinct = distinct,
								Exp = exp,
	...
	function S.HasSpawns(mobName)
		local cached = S.SpawnCache[mobName];
		if cached == nil then
			cached = #S.SpawnPoints(mobName) > 0;
			S.SpawnCache[mobName] = cached;
		end;
		return cached;
	end;

	function S.PickQuest(level)
		local ladder = S.BuildLadder();
		local best;
		for _, entry in ipairs(ladder) do
			if entry.LevelReq <= level - S.MinLevelGap and S.HasSpawns(entry.Mob) then
				local allowed = not (S.SkipBosses and entry.Boss);
				if allowed then
					if not best then
						best = entry;
					elseif S.Strategy == "Best XP Per Kill" then
						if entry.PerKill > best.PerKill then
							best = entry;
						end;
					elseif entry.LevelReq > best.LevelReq or (entry.LevelReq == best.LevelReq and entry.Exp > best.Exp) then
						best = entry;
					end;
				end;
			end;
		...
		if not best then
			for _, entry in ipairs(ladder) do
				if entry.LevelReq <= level and S.HasSpawns(entry.Mob) and (not best or entry.LevelReq > best.LevelReq) then
					best = entry;
				end;
			end;
		end;
	...
	function S.SpawnPoints(mobName)
		local origin = workspace:FindFirstChild("_WorldOrigin");
	...
	function S.BestCluster(mobName)
		local points = S.SpawnPoints(mobName);
		if #points == 0 then
			return nil, 0, 0;
		end;
		local radius = S.ClusterRadius;
		local bestCentre, bestCount;
		for _, anchor in ipairs(points) do
			local sum, count = Vector3.new(), 0;
			for _, other in ipairs(points) do
				if (other - anchor).Magnitude <= radius then
					sum = sum + other;
					count = count + 1;
				end;
			end;
		...
		return bestCentre, bestCount or 0, #points;
	end;
	...
	function S.LoadCache()
		...
		pcall(function()
			if isfile(S.CacheFile) then
				local decoded = game:GetService("HttpService"):JSONDecode(readfile(S.CacheFile));
	...
	function S.SaveCache()
		if not writefile then
			return;
		end;
		pcall(function()
			writefile(S.CacheFile, game:GetService("HttpService"):JSONEncode(S.NpcCache));
		end);
	end;

	function S.RememberGiver(questId, position)
		if not questId or typeof(position) ~= "Vector3" then
			return;
		end;
		local existing = S.NpcCache[questId];
		if existing and (Vector3.new(existing.x, existing.y, existing.z) - position).Magnitude < 12 then
			return;
		end;
		S.NpcCache[questId] = { x = position.X, y = position.Y, z = position.Z };
		S.SaveCache();
	end;

	function S.CachedGiver(questId)
		local entry = S.NpcCache[questId];
		if type(entry) == "table" and entry.x then
			return Vector3.new(entry.x, entry.y, entry.z);
		end;
		return nil;
	end;

	function S.FindGiver(questId, near)
		local cached = S.CachedGiver(questId);
		if cached then
			return cached, "cached";
		end;
		local holder = workspace:FindFirstChild("NPCs");
		if not holder then
			return nil, "no-npc-folder";
		end;
		local wanted = string.lower(string.gsub(questId, "Quest%d*$", ""));
		local best, bestDistance, bestWhy;
		for _, npc in ipairs(holder:GetChildren()) do
			...
			if root and string.find(string.lower(npc.Name), "quest giver", 1, true) then
				...
				if wanted ~= "" and label == wanted then
					S.RememberGiver(questId, root.Position);
				...
				if not best or distance < bestDistance then
					best, bestDistance, bestWhy = root.Position, distance, "nearest";
				end;
			end;
		end;
	...
	function S.SetStatus(text)
		...
		if text ~= S.Status then
			S.Status = text;
			if S.StatusLabel then
				S.StatusLabel:SetText("Better farm: " .. text:gsub("%-", " "));
			end;
		end;
	end;

	function S.ResetStats()
		S.Stats.StartExp = DataNumber("Exp");
		S.Stats.StartLevel = DataNumber("Level");
		S.Stats.StartClock = os.clock();
		S.Stats.LastExp = S.Stats.StartExp;
		S.Stats.LastLevel = S.Stats.StartLevel;
		S.Stats.Kills = 0;
	end;

	S.Gained = 0;
	function S.TickStats()
		local level = DataNumber("Level");
		local exp = DataNumber("Exp");
		if level > S.Stats.LastLevel then
			S.Gained = S.Gained + exp;
		elseif exp >= S.Stats.LastExp then
			S.Gained = S.Gained + (exp - S.Stats.LastExp);
		end;
		S.Stats.LastLevel = level;
		S.Stats.LastExp = exp;
	end;

	function S.ExpPerHour()
		local elapsed = os.clock() - (S.Stats.StartClock or os.clock());
		if elapsed < 5 then
			return 0;
		end;
	...
	function S.NextTier(level)
		local upcoming;
		for _, entry in ipairs(S.BuildLadder()) do
			if entry.LevelReq > level and (not upcoming or entry.LevelReq < upcoming.LevelReq) then
				upcoming = entry;
			end;
		end;
		return upcoming;
	...
	function S.Preview()
		...
		if level <= 0 then
			S.Choice = nil;
			S.Blocker = "Waiting for your player data to load.";
			S.SetStatus("waiting-for-data");
			return nil;
		end;
		local ladder = S.BuildLadder();
		if #ladder == 0 then
			S.Choice = nil;
			S.Blocker = "Could not read the game quest list. Press Rescan Game Data.";
			S.SetStatus("no-quest-list");
			return nil;
		end;
		local choice = S.PickQuest(level);
		if not choice then
			local farmable = 0;
			for _, entry in ipairs(ladder) do
				if S.HasSpawns(entry.Mob) then
					farmable = farmable + 1;
				end;
			end;
			S.Choice = nil;
			S.Blocker = string.format("Nothing fits level %d in this sea. %d quest tiers loaded, %d can be farmed here.", level, #ladder, farmable);
			S.SetStatus("no-quest-for-level");
			return nil;
		end;
		S.Choice = choice;
		S.Blocker = nil;
	...
	function S.Apply()
		local choice = S.Preview();
		if not choice then
			return false;
		end;
		local centre, clustered, total = S.BestCluster(choice.Mob);
		local sameAsLast = NameQuest == choice.Id and LevelQuest == choice.Tier;
		local giver, why = S.CachedGiver(choice.Id), "cached";
		if not giver and sameAsLast and typeof(CFrameQuest) == "CFrame" then
			giver, why = CFrameQuest.Position, "resolved-earlier";
		end;
		if not giver then
			giver, why = S.FindGiver(choice.Id, centre);
		end;
		if not giver then
			if centre then
				giver, why = centre, "cluster-fallback";
			else
				S.SetStatus("no-quest-giver");
				return false;
			end;
		end;
		Mon = choice.Mob;
		...
		S.LastPlan = string.format("%s #%d (Lv %d) - %s x%d - %s xp - %d/%d spawns - giver %s", choice.Id, choice.Tier, choice.LevelReq, choice.Mob, choice.Kills, Short(choice.Exp), clustered or 0, total or 0, tostring(why));
		S.SetStatus("targeting " .. choice.Mob);
		return true;
	end;

	function S.RefreshLabels()
		if not S.PlanLabel then
			return;
		end;
		local level = DataNumber("Level");
		local planText;
		if not S.Enabled then
			planText = "Better farm is off. The script uses its built in quest list.";
		else
			local ok, choice = pcall(S.Preview);
			if not ok then
				planText = "Better farm error: " .. tostring(choice);
			elseif not choice then
				planText = S.Blocker or "Working out the best quest...";
			else
				planText = string.format('<b>%s</b>  Lv %d\nKill <b>%s</b> x%d for <b>%s</b> xp  (%s per kill)', choice.Title, choice.LevelReq, choice.Mob, choice.Kills, Short(choice.Exp), Short(choice.PerKill));
				if not _G.Level then
					planText = planText .. '\n<font color="rgb(255, 205, 90)">Now turn on Auto Farm Level to start.</font>';
				end;
			end;
		end;
		if planText ~= S.LastPlanText then
			S.LastPlanText = planText;
			S.PlanLabel:SetText(planText);
		end;
		if S.StatsLabel then
			local perHour = S.ExpPerHour();
			local nextTier = S.NextTier(level);
			local etaText = "";
			if nextTier then
				etaText = string.format("\nNext quest at Lv %d  (%d levels away)", nextTier.LevelReq, nextTier.LevelReq - level);
			end;
			local statsText = string.format("Level %d  |  %s xp/hour%s", level, Short(perHour), etaText);
			if statsText ~= S.LastStatsText then
				S.LastStatsText = statsText;
...
UI.BuildSmartFarmTab = function()
	local S = UI.SmartFarm;
	local section = UI.Sections["BF/SmartFarm"];
	if not section then
		error("Better Farm section BF/SmartFarm was never created", 0);
	end;
	section:AddLabel({ DoesWrap = true, Text = "Reads the quest list and enemy spawn map straight out of the game, picks the best quest for your level and farms the busiest spawn point for it. Turn on Auto Farm Level as usual, this replaces the quest picking behind it." });
	section:AddToggle("BF_Toggle_Smart_Farm", {
		Text = "Better Auto Farm",
		Tooltip = "Pick quests and farming spots from live game data instead of the built in list",
		Default = false,
	...
	section:AddDropdown("BF_Dropdown_Smart_Farm_Strategy", {
		Text = "Pick Quests By",
		Tooltip = "Highest XP follows the normal ladder, Best XP Per Kill favours quests that finish fastest",
	...
	section:AddToggle("BF_Toggle_Smart_Farm_Skip_Bosses", {
		Text = "Skip Boss Quests",
		Tooltip = "Boss tiers are one kill but slow and risky, leave this on to stick to mob quests",
		Default = true,
		Callback = function(state)
			S.SkipBosses = state ~= false;
			S.Choice = nil;
			UI.AutoFarmNextQuestRefreshAt = 0;
			S.RefreshLabels();
		end,
	});
	section:AddSlider("BF_Slider_Smart_Farm_Cluster", {
		Text = "Spawn Cluster Size",
		Tooltip = "How wide a group of spawn points counts as one farming spot",
		Min = 100,
		Max = 1200,
		Default = 400,
		Rounding = 0,
		Callback = function(value)
			S.ClusterRadius = math.clamp(tonumber(value) or 400, 100, 1200);
			S.Choice = nil;
			UI.AutoFarmNextQuestRefreshAt = 0;
		end,
	});
	section:AddSlider("BF_Slider_Smart_Farm_Gap", {
		Text = "Stay Behind By",
		Tooltip = "Hold back this many levels before moving up a quest, useful if a tier kills you",
		Min = 0,
		Max = 200,
		Default = 0,
		Rounding = 0,
		Callback = function(value)
			S.MinLevelGap = math.max(0, math.floor(tonumber(value) or 0));
			S.Choice = nil;
			UI.AutoFarmNextQuestRefreshAt = 0;
		end,
	});
	S.PlanLabel = section:AddLabel({ DoesWrap = true, Text = "Better farm is off. The script uses its built in quest list." });
	S.StatusLabel = section:AddLabel({ DoesWrap = true, Text = "Better farm: idle" });
	S.StatsLabel = section:AddLabel({ DoesWrap = true, Text = "Level -  |  0 xp/hour" });
	section:AddButton({
		Text = "Show Full Plan",
		Tooltip = "Print what the farm picked and why",
		Func = function()
			UI.Library:Notify(S.LastPlan or "Nothing planned yet.", 8);
		end,
	});
	section:AddButton({
		Text = "Rescan Game Data",
		Tooltip = "Re-read the quest list and spawn map",
		Func = function()
			S.BuildLadder(true);
			S.Choice = nil;
			UI.AutoFarmNextQuestRefreshAt = 0;
			UI.Library:Notify(string.format("Loaded %d quest tiers.", #S.Ladder), 4);
			S.RefreshLabels();
		end,
	});

	if type(CheckQuest) == "function" and not S.WrappedCheckQuest then
		S.OriginalCheckQuest = CheckQuest;
		S.WrappedCheckQuest = true;
		CheckQuest = function(...)
			if S.Enabled then
				local ok, applied = pcall(S.Apply);
				if ok and applied then
					return;
				end;
				if not ok then
					S.LastPlan = "Better farm error: " .. tostring(applied);
				end;
				S.SetStatus("falling-back");
			end;
			return S.OriginalCheckQuest(...);
		end;
	end;
	...
	task.spawn(function()
		while not UI.Stopped and task.wait(1) do
			...
			if S.Enabled then
				pcall(S.TickStats);
				if S.Choice and _G.Level and UI.AutoFarmStatus == "quest-started" then
					local root = d.Character and d.Character:FindFirstChild("HumanoidRootPart");
					if root then
						pcall(S.RememberGiver, S.Choice.Id, root.Position);
					end;
				end;
			end;
			pcall(S.RefreshLabels);
		end;
	end);
end;
do
	local ok, err = pcall(UI.BuildSmartFarmTab);
	if not ok then
		task.spawn(function()
			UI.Library:Notify("Failed to build Better Auto Farm: " .. tostring(err), 10);
		end);
	end;
end;
UI.MoneyFarm = {
	Enabled = false,
	Sources = { Chests = true, Bosses = true, ["Beli Quests"] = true },
	GrabRadius = 900,
	HopWhenEmpty = false,
	MinChests = 3,
	Status = "idle",
	...
	NextQuestAt = 0,
	NextHopAt = 0,
	Stats = { StartBeli = 0, StartClock = 0, LastBeli = 0, Gained = 0, Chests = 0, Bosses = 0 },
};
do
	local M = UI.MoneyFarm;
	local S = UI.SmartFarm;

	local function Beli()
		return tonumber(BFDataValue("Beli")) or 0;
	end;
	...
	function M.SetStatus(text)
		text = tostring(text or "idle");
		if text ~= M.Status then
			M.Status = text;
			if M.StatusLabel then
				M.StatusLabel:SetText("Money farm: " .. text:gsub("%-", " "));
			end;
		end;
	end;

	function M.TweenTo(target, tolerance)
		local goal = target;
		...
		if (goal.Position - root.Position).Magnitude <= (tonumber(tolerance) or 8) then
			return true;
		end;
		_tp(goal, true);
		return false;
	...
	function M.NearestChest(from, maxDistance)
		local best, bestDistance;
		for _, chest in ipairs(M.LiveChests()) do
			local ok, pivot = pcall(chest.GetPivot, chest);
			if ok then
				local distance = (pivot.Position - from).Magnitude;
				if (not maxDistance or distance <= maxDistance) and (not bestDistance or distance < bestDistance) then
	...
	function M.BossNames()
		if M.BossCache and os.clock() < (M.BossCacheAt or 0) + 120 then
			return M.BossCache;
		end;
		local origin = workspace:FindFirstChild("_WorldOrigin");
		local holder = origin and origin:FindFirstChild("EnemySpawns");
		...
		if holder then
			for _, part in ipairs(holder:GetChildren()) do
				if string.find(part.Name, "%[Boss%]") then
					local plain = string.match(part.Name, "^(.-)%s*%[Lv%.");
					if plain and not names[plain] then
						names[plain] = true;
					end;
				end;
			end;
		...
		M.BossCache = names;
		M.BossCacheAt = os.clock();
		return names;
	end;

	function M.LiveBoss()
		local holder = workspace:FindFirstChild("Enemies");
		if not holder then
			return nil;
		end;
		local bosses = M.BossNames();
		local level = tonumber(BFDataValue("Level")) or 0;
		local best, bestLevel;
		for _, enemy in ipairs(holder:GetChildren()) do
			local plain = string.match(enemy.Name, "^(.-)%s*%[Lv%.") or enemy.Name;
			if bosses[plain] then
				local humanoid = enemy:FindFirstChild("Humanoid");
				local root = enemy:FindFirstChild("HumanoidRootPart");
				if humanoid and root and humanoid.Health > 0 then
					local bossLevel = tonumber(string.match(enemy.Name, "%[Lv%.%s*(%d+)")) or 0;
					if bossLevel <= level * 1.5 and (not bestLevel or bossLevel > bestLevel) then
						best, bestLevel = enemy, bossLevel;
					end;
				end;
			end;
		end;
	...
	function M.BestBeliQuest()
		local level = tonumber(BFDataValue("Level")) or 0;
		local ladder = S.BuildLadder();
		local best;
		for _, entry in ipairs(ladder) do
			if entry.LevelReq <= level and entry.Beli > 0 and S.HasSpawns(entry.Mob) then
				local perKill = entry.Beli / entry.Kills;
				if not best or perKill > best.PerBeli then
					best = { Entry = entry, PerBeli = perKill };
				end;
			end;
		end;
	...
	function M.ResetStats()
		M.Stats.StartBeli = Beli();
		M.Stats.LastBeli = M.Stats.StartBeli;
		M.Stats.StartClock = os.clock();
		M.Stats.Gained = 0;
		M.Stats.Chests = 0;
		M.Stats.Bosses = 0;
	end;

	function M.TickStats()
		local now = Beli();
		if now > M.Stats.LastBeli then
			M.Stats.Gained = M.Stats.Gained + (now - M.Stats.LastBeli);
		end;
		M.Stats.LastBeli = now;
	end;

	function M.BeliPerHour()
		local elapsed = os.clock() - (M.Stats.StartClock or os.clock());
		if elapsed < 5 then
			return 0;
		end;
		return M.Stats.Gained / elapsed * 3600;
	end;

	function M.Step()
		if not _G.MoneyFarm then
			M.Target = nil;
			return "inactive";
		end;
		local root = Root();
		...
		local wantChests = M.Sources.Chests == true;
		local wantBosses = M.Sources.Bosses == true;
		local wantQuests = M.Sources["Beli Quests"] == true;

		if wantChests then
			local chest, distance = M.NearestChest(here, M.GrabRadius);
			if chest then
				M.Target = chest;
				M.Plan = string.format("Chest %d studs away", math.floor(distance or 0));
				if M.TweenTo(chest:GetPivot(), 6) then
					M.Stats.Chests = M.Stats.Chests + 1;
					return "opening-chest";
				end;
				return "tweening-to-chest";
			end;
		end;

		if wantBosses then
			local boss = M.LiveBoss();
			if boss then
				M.Target = boss;
				M.Plan = "Boss: " .. boss.Name;
				f.Kill(boss, _G.MoneyFarm);
				M.Stats.Bosses = M.Stats.Bosses + 1;
				return "fighting-boss";
			end;
		end;

		if wantQuests then
			local quest = M.BestBeliQuest();
			if quest then
				M.Plan = string.format("%s - %s x%d for %s beli", quest.Id, quest.Mob, quest.Kills, S.Short(quest.Beli));
				local enemy = BFFindLiveEnemyLike(quest.Mob);
				if enemy then
					M.Target = enemy;
					f.Kill(enemy, _G.MoneyFarm);
					return "farming-beli-quest";
				end;
				local centre = S.BestCluster(quest.Mob);
				if not CheckHasQuest(quest.Mob) and os.clock() >= M.NextQuestAt then
					local giver = S.CachedGiver(quest.Id) or S.FindGiver(quest.Id, centre);
					if giver and M.TweenTo(giver, 4) then
						BFComm("StartQuest", quest.Id, quest.Tier);
						M.NextQuestAt = os.clock() + 2;
						return "starting-beli-quest";
					end;
					if giver then
						return "tweening-to-quest-giver";
					end;
				end;
				if centre then
					M.TweenTo(centre, 40);
					return "tweening-to-beli-mobs";
				end;
			end;
		end;

		if wantChests then
			local chest = M.NearestChest(here, nil);
			if chest then
				M.Target = chest;
				M.Plan = "Tweening to a far chest";
				if M.TweenTo(chest:GetPivot(), 6) then
					M.Stats.Chests = M.Stats.Chests + 1;
					return "opening-chest";
				end;
			...
			local remaining = #M.LiveChests();
			if M.HopWhenEmpty and remaining <= M.MinChests and os.clock() >= M.NextHopAt then
				M.NextHopAt = os.clock() + 20;
				M.Plan = "Map is out of chests, hopping server";
	...
	function M.RefreshLabels()
		if M.PlanLabel then
			local text;
			if not _G.MoneyFarm then
				text = "Money farm is off.";
			else
				text = M.Plan ~= "" and M.Plan or "Looking for something to do...";
			end;
			if text ~= M.LastPlanText then
				M.LastPlanText = text;
				M.PlanLabel:SetText(text);
			end;
		end;
		if M.StatsLabel then
			local text = string.format("%s beli/hour  |  %s earned  |  %d chests  |  %d bosses", S.Short(M.BeliPerHour()), S.Short(M.Stats.Gained), M.Stats.Chests, M.Stats.Bosses);
			if text ~= M.LastStatsText then
				M.LastStatsText = text;
				M.StatsLabel:SetText(text);
			end;
		end;
		if M.ChestLabel then
			local text = string.format("%d chests left on this server", #M.LiveChests());
			if text ~= M.LastChestText then
				M.LastChestText = text;
				M.ChestLabel:SetText(text);
			end;
		end;
	end;
end;
UI.RegisterManagedFlag("MoneyFarm", false);
UI.BuildMoneyFarmTab = function()
	local M = UI.MoneyFarm;
	local section = UI.Sections["BF/MoneyFarm"];
	if not section then
		error("Money Farm section BF/MoneyFarm was never created", 0);
	end;
	section:AddLabel({ DoesWrap = true, Text = "Earns beli from several things at once. It grabs any chest you pass on the way to whatever else it is doing, fights money bosses when they are up, and farms the best paying quest for your level in between." });
	section:AddToggle("BF_Toggle_Money_Farm", {
		Text = "Money Farm",
		Tooltip = "Run the beli rotation",
		Default = false,
		Callback = function(state)
			UI.SetManagedUserFlag("MoneyFarm", state);
			if state then
				M.ResetStats();
				M.NextQuestAt = 0;
				M.NextHopAt = 0;
	...
	section:AddDropdown("BF_Dropdown_Money_Sources", {
		Text = "Earn From",
		Tooltip = "Pick every money source you want running together",
		Values = { "Chests", "Bosses", "Beli Quests" },
		Default = { "Chests", "Bosses", "Beli Quests" },
		Multi = true,
		NoMode = true,
	...
	section:AddSlider("BF_Slider_Money_Grab_Radius", {
		Text = "Grab Chests Within",
		Tooltip = "Any chest closer than this gets picked up while doing something else",
		Min = 0,
		Max = 5000,
		Default = 900,
		Rounding = 0,
		Callback = function(value)
			M.GrabRadius = math.max(0, tonumber(value) or 900);
		end,
	});
	section:AddToggle("BF_Toggle_Money_Hop", {
		Text = "Hop When Chests Run Out",
		Tooltip = "Change server once the map has been cleared of chests",
		Default = false,
		Callback = function(state)
			M.HopWhenEmpty = state == true;
		end,
	});
	section:AddSlider("BF_Slider_Money_Min_Chests", {
		Text = "Hop Below",
		Tooltip = "How few chests must be left before hopping",
		Min = 0,
		Max = 30,
		Default = 3,
		Rounding = 0,
		Suffix = " chests",
		Callback = function(value)
			M.MinChests = math.max(0, math.floor(tonumber(value) or 3));
		end,
	});
	M.PlanLabel = section:AddLabel({ DoesWrap = true, Text = "Money farm is off." });
	M.StatusLabel = section:AddLabel({ DoesWrap = true, Text = "Money farm: idle" });
	M.ChestLabel = section:AddLabel({ DoesWrap = true, Text = "0 chests left on this server" });
	M.StatsLabel = section:AddLabel({ DoesWrap = true, Text = "0 beli/hour  |  0 earned  |  0 chests  |  0 bosses" });
	section:AddButton({
		Text = "Reset Counters",
		Tooltip = "Start the beli per hour timer again",
		Func = function()
			M.ResetStats();
			M.RefreshLabels();
		end,
	...
	task.spawn(function()
		while IdleWait(_G.MoneyFarm, .2) do
			if _G.MoneyFarm then
				local ok, status = pcall(M.Step);
				if ok then
					M.SetStatus(status);
				else
					M.SetStatus("error");
					M.Plan = tostring(status);
				end;
			end;
		end;
	end);
	task.spawn(function()
		while not UI.Stopped and task.wait(1) do
			...
			pcall(M.RefreshLabels);
		end;
	end);
end;
do
	local ok, err = pcall(UI.BuildMoneyFarmTab);
	if not ok then
		task.spawn(function()
			UI.Library:Notify("Failed to build Money Farm: " .. tostring(err), 10);
		end);
	end;
end;
...
-- ── Island Secrets ────────────────────────────────────────────────────────────
do
	local sec = UI.Sections["Island Secrets"];
	UI.RegisterManagedFlag("AutoIslandSecret", false);

	-- Island center positions from _WorldOrigin.Locations (live-probed).
	local ISLAND_POS = {
		Desert        = Vector3.new(1194, 20, 4430),
		PirateVillage = Vector3.new(-1133, 20, 4199),
		Colosseum     = Vector3.new(-1666, 20, -3242),
		MagmaVillage  = Vector3.new(-5528, 20, 8691),
		Prison        = Vector3.new(5278, 20, 743),
		FrozenVillage = Vector3.new(1277, 20, -1473),
		Jungle        = Vector3.new(-1516, 20, -130),
	};

	-- Desert Rescue Hasan coffin positions (live-probed from workspace.Map.Desert.Rescue Hasan).
	local HASAN_COFFIN_POS = {
		Vector3.new(1354, 27, 4416),
		Vector3.new(1377, 27, 4467),
		Vector3.new(1319, 27, 4404),
		Vector3.new(1269, 27, 4428),
	};
	...
	-- Find nearest alive enemy (workspace.Characters or workspace.Enemies) within radius of a Vector3 center.
	local function findNearestInRadius(center, radius, nameFilter)
		local nearest, nearestDist = nil, math.huge;
				...
				for _, model in ipairs(container:GetChildren()) do
					...
					if hrp and h and h.Health > 0 then
						...
						if distToCenter <= radius then
							if not nameFilter or nameFilter[model.Name] then
								local distToMe = (hrp.Position - (BFCharacterPart() and BFCharacterPart().Position or Vector3.zero)).Magnitude;
								if distToMe < nearestDist then
									nearest = model;
	...
	local SECRETS = {
		...
		},
		{
			...
			Run = function(statusLabel)
				...
				if (root.Position - ISLAND_POS.PirateVillage).Magnitude > 50 then
					statusLabel:SetText("Status: travelling to Pirate Village");
		...
		},
		{
			...
			Run = function(statusLabel)
				...
				if not root then return end;
				if (root.Position - ISLAND_POS.PirateVillage).Magnitude > 50 then
					statusLabel:SetText("Status: travelling to Pirate Village tavern");
					_tp(CFrame.new(ISLAND_POS.PirateVillage.X, ISLAND_POS.PirateVillage.Y + 15, ISLAND_POS.PirateVillage.Z), true);
					task.wait(2);
				end;
		...
		},
		{
			...
			Run = function(statusLabel)
				...
				if (root.Position - ISLAND_POS.Colosseum).Magnitude > 50 then
					statusLabel:SetText("Status: travelling to Colosseum");
				...
				statusLabel:SetText("Status: Colosseum done – check star! (" .. killed .. " kills)");
			end,
		},
		{
			...
			Run = function(statusLabel)
				...
				if (root.Position - ISLAND_POS.MagmaVillage).Magnitude > 60 then
					statusLabel:SetText("Status: travelling to Magma Village");
		...
		},
		{
			...
			Run = function(statusLabel)
				...
				if (root.Position - ISLAND_POS.Prison).Magnitude > 60 then
					statusLabel:SetText("Status: travelling to Prison");
		...
		},
		{
			...
			Run = function(statusLabel)
				...
				if (root.Position - ISLAND_POS.FrozenVillage).Magnitude > 60 then
					statusLabel:SetText("Status: travelling to Frozen Village");
	...
	for _, s in ipairs(SECRETS) do
		table.insert(secretNames, s.Name);
		secretMap[s.Name] = s;
	end;

	local selectedSecret = SECRETS[1];
	local statusLabel = sec:AddLabel({ DoesWrap = true, Text = "Status: idle" });

	sec:AddDropdown("BF_IslandSecret_Select", {
		Values = secretNames,
		Default = secretNames[1],
		Text = "Select Secret",
		Tooltip = "Choose which Sea 1 island secret to automate",
		Callback = function(val)
			selectedSecret = secretMap[val] or SECRETS[1];
			statusLabel:SetText("Selected: " .. selectedSecret.Name);
		end,
	});

	sec:AddLabel({ DoesWrap = true, Text = "Teleports to the island, triggers the secret, kills required enemies. Watch the star counter on your world map." });

	sec:AddToggle("BF_Toggle_Auto_Island_Secret", {
		Text = "Run Selected Secret",
		Tooltip = "Automate the currently selected island secret",
		Default = false,
		Callback = function(Y)
			UI.SetManagedUserFlag("AutoIslandSecret", Y);
			if not Y then statusLabel:SetText("Status: idle") end;
		end,
	});
...
local BFBlackLegNPCCFrame = CFrame.new(-1323.5, 25.8, 1025.2, -0.342, 0, 0.940, 0, 1, 0, -0.940, 0, -0.342);
function UI.StyleMasteryStep(owner, spec, masteryRequired)
	...
	if not GetIn(spec.Name) then
		...
		-- Navigate to fighting style NPC with forced tween before buying.
		if UI.StyleRequestReady(owner, 2) then
			if spec.Command == "BuyBlackLeg" and World1 then
				if not BFMoveNear(BFBlackLegNPCCFrame, 8) then
					_tp(BFBlackLegNPCCFrame, true);
					task.wait(.5);
				end;
			end;
	...
	if tab then
		...
		if queue_on_teleport then
			tab:AddToggle("BF_AutoExecute", {
				...
				Callback = function(state)
					if state then
						pcall(queue_on_teleport, "loadstring(game:HttpGet('https://luminon.top/loader.lua'))()");
					else
		...
		do
			local autoJoinConn;
			tab:AddToggle("BF_AutoJoinPirates", {
				Text = "Auto Join Pirates",
				Default = false,
				Tooltip = "Automatically picks Pirates on the team selection screen",
				Callback = function(state)
					if autoJoinConn then
						autoJoinConn:Disconnect();
						autoJoinConn = nil;
					end;
					if not state then return; end;
					local function tryJoin()
						local gui = d.PlayerGui:FindFirstChild("TeamSelection");
						if gui and gui.Enabled then
							BFComm("SetTeam", "Pirates");
						end;
					end;
					tryJoin();
					autoJoinConn = d.PlayerGui.ChildAdded:Connect(function(child)
						if child.Name == "TeamSelection" then
							task.wait(0.3);
							tryJoin();
						end;
					end);
					UI.Library:GiveSignal(autoJoinConn);
				end,
			});
		end;
		tab:AddDivider();
...
UI.Priority.Catalog = {
	{ Id = "raidstart", Name = "Start Raid", Toggle = "BF_Toggle_Auto_Start_Raid", Flags = { "Auto_StartRaid" }, Seas = { 2, 3 }, Note = "Chip raids, timed" },
	{ Id = "pirateraid", Name = "Pirate Raid", Toggle = "BF_Toggle_Auto_Pirate_Raid", Flags = { "AutoRaidCastle" }, Seas = { 3 }, Note = "Castle on the Sea" },
	{ Id = "factory", Name = "Factory Raid", Toggle = "BF_Toggle_Auto_Factory_Raid", Flags = { "AutoFactory", "FactoryRaids" }, Seas = { 2 }, Note = "Factory core spawns" },
	{ Id = "seabeast", Name = "Sea Beast", Toggle = "BF_Toggle_Auto_Attack_Selected_Sea_Targets", Flags = { "SeaBeast1" }, Seas = { 2, 3 }, Note = "Also spawns in Sea 2" },
	{ Id = "leviathan", Name = "Leviathan", Toggle = "BF_Toggle_Auto_Attack_Selected_Sea_Targets", Flags = { "Leviathan1" }, Seas = { 3 }, Note = "Third Sea only" },
	{ Id = "seamobs", Name = "Sea Event Mobs", Toggle = "BF_Toggle_Auto_Attack_Selected_Sea_Targets", Flags = { "Shark", "Piranha", "TerrorShark", "MobCrew", "HCM", "PGB", "FishBoat" }, Seas = { 2, 3 }, Note = "Sharks, crews, brigades" },
	{ Id = "sailboat", Name = "Sail Boat", Toggle = "BF_Toggle_Auto_Sail_Boat", Flags = { "SailBoats" }, Seas = { 2, 3 }, Note = "Sails for sea events" },
	{ Id = "elite", Name = "Elite Hunt", Toggle = "BF_Toggle_Auto_Elite_Quest", Flags = { "FarmEliteHunt" }, Seas = { 3 }, Note = "Elite Hunter quest" },
	{ Id = "miragechest", Name = "Mirage Chest", Toggle = "BF_Toggle_Auto_Collect_Mirage_Chest", Flags = { "FarmChestM" }, Seas = { 3 }, Note = "Mirage Island only" },
	{ Id = "dinobone", Name = "Dino Bones", Toggle = "BF_Toggle_Auto_Collect_Dino_Bones", Flags = { "Prehis_DB" }, Seas = { 3 }, Note = "Prehistoric Island" },
	{ Id = "bones", Name = "Bone Farm", Toggle = "BF_Toggle_Auto_Bones", Flags = { "AutoFarm_Bone" }, Seas = { 3 }, Note = "Haunted Castle mobs" },
	{ Id = "skullguitar", Name = "Skull Guitar Mats", Toggle = "BF_Toggle_Auto_Farm_Material_Skull_Guitar", Flags = { "AutoMatSoul" }, Seas = { 3 }, Note = "Ecto, fragment, bones" },
	{ Id = "ectoplasm", Name = "Ectoplasm Farm", Toggle = "BF_Toggle_Auto_Farm_Ectoplasm", Flags = { "AutoEctoplasm" }, Seas = { 3 }, Note = "Ship crew mobs" },
	{ Id = "materials", Name = "Material Farm", Toggle = "BF_Toggle_Auto_Materials", Flags = { "AutoMaterial" }, Seas = { 1, 2, 3 }, Note = "Selected material mobs" },
	{ Id = "chest", Name = "Chest Farm", Toggle = "BF_Toggle_Auto_Collect_Chest", Flags = { "AutoFarmChest" }, Seas = { 1, 2, 3 }, Note = "Chest collecting" },
	{ Id = "money", Name = "Money Farm", Toggle = "BF_Toggle_Money_Farm", Flags = { "MoneyFarm" }, Seas = { 1, 2, 3 }, Note = "Chests, bosses and beli quests" },
	{ Id = "zou", Name = "Zou Quest", Toggle = "BF_Toggle_Auto_Zou_Quest", Flags = { "AutoZou", "Zou" }, Seas = { 2 }, Note = "Second Sea progression" },
	{ Id = "bartilo", Name = "Bartilo Quest", Toggle = "BF_Toggle_Auto_Done_Bartilo_Quest", Flags = { "Bartilo_Quest" }, Seas = { 1 }, Note = "First Sea only" },
	{ Id = "citizen", Name = "Citizen Quest", Toggle = "BF_Toggle_Auto_Done_Citizen_Quest", Flags = { "CitizenQuest" }, Seas = { 1 }, Note = "First Sea only" },
	{ Id = "mastery", Name = "Mastery Farm", Toggle = "BF_Toggle_Auto_Mastery_Selected", Flags = { "FarmMastery_Dev", "FarmMastery_G", "FarmMastery_S" }, Seas = { 1, 2, 3 }, Note = "Selected weapon mastery" },
	{ Id = "level", Name = "Level Farm", Toggle = "BF_Toggle_Auto_Farm_Level", Flags = { "Level" }, Seas = { 1, 2, 3 }, Note = "Main quest grind" },
	{ Id = "observation", Name = "Observation Farm", Toggle = "BF_Toggle_Auto_Farm_Observation", Flags = { "obsFarm" }, Seas = { 1, 2, 3 }, Note = "Instinct levelling" },
	{ Id = "fishing", Name = "Fishing", Toggle = "BF_Toggle_Auto_Fishing", Flags = { "AutoFishing" }, Seas = { 1, 2, 3 }, Note = "Auto fishing loop" },
	{ Id = "nearest", Name = "Farm Nearest", Toggle = "BF_Toggle_Auto_Farm_Nearest", Flags = { "AutoFarmNear" }, Seas = { 1, 2, 3 }, Note = "Attack closest enemy" },
};
UI.Priority.Presets = {
	["Default"] = {},
	["Events First"] = { "raidstart", "pirateraid", "factory", "seabeast", "leviathan", "seamobs", "sailboat", "elite", "miragechest" },
	["Leveling First"] = { "level", "mastery", "elite", "zou", "bartilo", "citizen", "raidstart" },
	["Loot First"] = { "chest", "miragechest", "materials", "bones", "dinobone", "skullguitar", "ectoplasm", "fishing" },
};
UI.Priority.PresetNames = { "Default", "Events First", "Leveling First", "Loot First" };
do
	local P = UI.Priority;
	local SEA_NAMES = { [1] = "First Sea", [2] = "Second Sea", [3] = "Third Sea" };
	local COLOR_ON = "rgb(90, 235, 130)";
	local COLOR_QUEUE = "rgb(255, 205, 90)";
	local COLOR_LOCK = "rgb(255, 110, 110)";
	local COLOR_OFF = "rgb(150, 150, 160)";
	local COLOR_ACCENT = "rgb(199, 0, 255)";
	local function Paint(color, text)
		return string.format('<font color="%s">%s</font>', color, tostring(text));
	end;
	function P.CurrentSea()
	...
	function P.SeaLabel(sea)
		return SEA_NAMES[sea] or "Unknown Sea";
	end;
	function P.SeaListText(task)
		if not task.Seas then
			return "Any sea";
		end;
		local parts = {};
		for sea = 1, 3 do
			if task.SeaSet[sea] then
				table.insert(parts, "Sea " .. sea);
			end;
		end;
		if #parts == 0 then
			return "Any sea";
	...
	local function FlagManaged(name)
		return UI.ManagedFlags[name] ~= nil;
	end;
	local function ToggleOf(task)
		if not task.Toggle then
			return nil;
		end;
		return UI.Toggles[task.Toggle];
	end;
	function P.TaskSupported(task)
		return ToggleOf(task) ~= nil;
	end;
	function P.TaskWanted(task)
		local toggle = ToggleOf(task);
		if not toggle or toggle.Value ~= true then
			return false;
		end;
		local managedSeen = false;
		for _, flag in ipairs(task.Flags) do
			local state = UI.ManagedFlags[flag];
			if state then
				managedSeen = true;
				if state.UserValue == true then
					return true;
				end;
			end;
		end;
	...
	function P.TaskAvailable(task, sea)
		if not P.SeaLock then
			return true;
		end;
		if sea == 0 or not task.SeaSet then
			return true;
		end;
		return task.SeaSet[sea] == true;
	end;
	function P.HealthPercent()
		local humanoid;
		if type(BFHumanoid) == "function" then
			local ok, result = pcall(BFHumanoid);
			if ok then
				humanoid = result;
			end;
		end;
	...
	function P.HealthBlocked()
		if not P.LowHealthPause then
			return false;
		end;
		local percent = P.HealthPercent();
		if not percent then
			return false;
		end;
		return percent <= P.HealthPct;
	end;
	function P.HoldTask(task)
		for _, flag in ipairs(task.Flags) do
			if FlagManaged(flag) then
				UI.SuppressManagedFlag(P.Owner, flag);
			else
				_G[flag] = false;
			end;
		end;
	...
	function P.ReleaseTask(task)
		P.Held[task.Id] = nil;
		for _, flag in ipairs(task.Flags) do
			if FlagManaged(flag) then
				UI.ReleaseManagedFlag(P.Owner, flag);
			end;
		end;
		local toggle = ToggleOf(task);
		if toggle and toggle.Value == true and type(toggle.SetValue) == "function" then
			pcall(toggle.SetValue, toggle, true);
		end;
	end;
	function P.ReleaseAll()
		for _, task in ipairs(P.Catalog) do
			if P.Held[task.Id] then
				P.ReleaseTask(task);
			end;
		end;
		UI.ReleaseManagedOwner(P.Owner);
	end;
	function P.NormaliseOrder(order)
		local seen = {};
		local result = {};
		for _, id in ipairs(order or {}) do
			if P.Tasks[id] and not seen[id] then
				seen[id] = true;
	...
	function P.SetOrder(order, skipInput)
		P.Order = P.NormaliseOrder(order);
		for index, id in ipairs(P.Order) do
			P.Tasks[id].Rank = index;
		end;
		if not skipInput then
			local input = P.Controls.OrderInput;
			if input and type(input.SetValue) == "function" then
				P.SuppressOrderInput = true;
				pcall(input.SetValue, input, table.concat(P.Order, ","));
				P.SuppressOrderInput = false;
			end;
		end;
		P.Dirty = true;
	end;
	function P.Move(id, delta)
		if not id or not P.Tasks[id] then
			return false;
		end;
		local index;
		for position, value in ipairs(P.Order) do
			if value == id then
				index = position;
				break;
			end;
		...
		local target = math.clamp(index + delta, 1, #P.Order);
		if target == index then
			return false;
		end;
		table.remove(P.Order, index);
		table.insert(P.Order, target, id);
		P.SetOrder(P.Order);
		return true;
	end;
	function P.MoveTo(id, position)
		if not id or not P.Tasks[id] then
			return false;
		end;
		local index;
		for slot, value in ipairs(P.Order) do
			if value == id then
				index = slot;
				break;
			end;
		...
		table.remove(P.Order, index);
		table.insert(P.Order, math.clamp(position, 1, #P.Order + 1), id);
		P.SetOrder(P.Order);
		return true;
	end;
	function P.ApplyPreset(name)
		local preset = P.Presets[name];
		if not preset then
			return;
		end;
		P.SetOrder(preset);
		UI.Library:Notify("Priority preset: " .. tostring(name), 3);
	end;
	function P.Step()
		local sea = P.CurrentSea();
		P.Sea = sea;
		local blocked = P.HealthBlocked();
		P.Blocked = blocked;
		local activeNames = {};
		local queuedNames = {};
		local used = 0;
		for _, task in ipairs(P.Catalog) do
			task.Supported = P.TaskSupported(task);
			task.Wanted = task.Supported and P.TaskWanted(task);
			task.Available = P.TaskAvailable(task, sea);
			task.State = "off";
		end;
		for _, id in ipairs(P.Order) do
			local task = P.Tasks[id];
			if task and task.Wanted then
				if not task.Available then
					task.State = "sea-locked";
				elseif blocked then
					task.State = "paused";
				elseif not P.Enabled then
					task.State = "active";
				elseif used < P.Slots then
					used = used + 1;
					task.State = "active";
				else
					task.State = "queued";
				...
				if task.State == "active" then
					table.insert(activeNames, task.Name);
				elseif task.State == "queued" or task.State == "paused" then
					table.insert(queuedNames, task.Name);
				end;
			end;
		end;
		for _, task in ipairs(P.Catalog) do
			local shouldHold = P.Enabled and task.Wanted and task.State ~= "active";
			if shouldHold then
				P.HoldTask(task);
			elseif P.Held[task.Id] then
				P.ReleaseTask(task);
			end;
		end;
		P.ActiveNames = activeNames;
		P.QueuedNames = queuedNames;
	end;
	function P.BuildOrderText()
		local filter = P.ViewSea;
		local lines = {};
		local hidden = 0;
		for index, id in ipairs(P.Order) do
			local task = P.Tasks[id];
			local matchesSea = task and (filter == 0 or not task.SeaSet or task.SeaSet[filter]);
			if task and matchesSea and P.OnlyEnabled and not task.Wanted then
				hidden = hidden + 1;
				matchesSea = false;
			end;
			if task and matchesSea then
				local marker, color, suffix;
				if not task.Supported then
					marker, color, suffix = "-", COLOR_OFF, "unavailable";
				elseif task.State == "active" then
					marker, color, suffix = "#", COLOR_ON, "running";
				elseif task.State == "queued" then
					marker, color, suffix = "+", COLOR_QUEUE, "waiting";
				elseif task.State == "paused" then
					marker, color, suffix = "!", COLOR_QUEUE, "held, low hp";
				elseif task.State == "sea-locked" then
					marker, color, suffix = "x", COLOR_LOCK, P.SeaListText(task) .. " only";
				else
					marker, color, suffix = ".", COLOR_OFF, P.SeaListText(task);
				end;
				table.insert(lines, string.format("%s %02d %s  %s", Paint(color, marker), index, task.Name, Paint(color, suffix)));
			end;
		end;
		if #lines == 0 then
			if hidden > 0 then
				return "Nothing switched on yet. Turn a feature on anywhere in the menu and it shows up here.";
			end;
			return "No tasks match this filter.";
		end;
		if hidden > 0 then
			table.insert(lines, Paint(COLOR_OFF, string.format("...and %d switched off", hidden)));
		end;
		return table.concat(lines, "\n");
	end;
	function P.Refresh()
		local orderText = P.BuildOrderText();
		if orderText ~= P.LastOrderText then
			P.LastOrderText = orderText;
			if P.Controls.OrderLabel then
				P.Controls.OrderLabel:SetText(orderText);
			end;
		end;
		local activeText;
		if not P.Enabled then
			activeText = "Engine off - every enabled feature runs at once.";
		elseif P.Blocked then
			activeText = Paint(COLOR_QUEUE, "Paused - health below " .. tostring(P.HealthPct) .. "%");
		elseif P.ActiveNames and #P.ActiveNames > 0 then
			activeText = "Running: " .. Paint(COLOR_ON, table.concat(P.ActiveNames, ", "));
		else
			activeText = "Running: " .. Paint(COLOR_OFF, "nothing");
		end;
		local queuedText;
		if P.QueuedNames and #P.QueuedNames > 0 then
			queuedText = "Waiting: " .. Paint(COLOR_QUEUE, table.concat(P.QueuedNames, ", "));
		else
			queuedText = "Waiting: " .. Paint(COLOR_OFF, "nothing");
		end;
		if activeText ~= P.LastActiveText then
			P.LastActiveText = activeText;
			if P.Controls.ActiveLabel then
				P.Controls.ActiveLabel:SetText(activeText);
			end;
		end;
		if queuedText ~= P.LastQueuedText then
			P.LastQueuedText = queuedText;
			if P.Controls.QueuedLabel then
				P.Controls.QueuedLabel:SetText(queuedText);
			end;
		end;
		local seaText = string.format("You are in %s%s", Paint(COLOR_ACCENT, P.SeaLabel(P.Sea)), P.SeaLock and "" or "  (sea lock off)");
		if seaText ~= P.LastSeaText then
			P.LastSeaText = seaText;
			if P.Controls.SeaLabel then
				P.Controls.SeaLabel:SetText(seaText);
			end;
		end;
		local wrongSea = {};
		for _, id in ipairs(P.Order) do
			local task = P.Tasks[id];
			if task and task.Wanted and task.State == "sea-locked" then
				table.insert(wrongSea, task.Name .. " (" .. P.SeaListText(task) .. ")");
			end;
		end;
		local wrongText;
		if #wrongSea == 0 then
			wrongText = "Every enabled task can run in this sea.";
		else
			wrongText = "Skipped here: " .. Paint(COLOR_LOCK, table.concat(wrongSea, ", "));
		end;
		if wrongText ~= P.LastWrongText then
			P.LastWrongText = wrongText;
			if P.Controls.WrongSeaLabel then
				P.Controls.WrongSeaLabel:SetText(wrongText);
			end;
		end;
		local warningTitle, warningBody;
		if not P.Enabled then
			warningTitle = "Priority Off";
			warningBody = "Turn the engine on to make the script run one job at a time, top of the list first.";
		else
			local headline = P.ActiveNames and P.ActiveNames[1] or nil;
			warningTitle = headline and ("Running: " .. headline) or "Nothing Running";
			warningBody = string.format("%s  |  %d slot%s  |  %d waiting", P.SeaLabel(P.Sea), P.Slots, P.Slots == 1 and "" or "s", P.QueuedNames and #P.QueuedNames or 0);
		end;
		local stamp = warningTitle .. "|" .. warningBody;
		if stamp ~= P.LastWarningStamp then
			P.LastWarningStamp = stamp;
			pcall(function()
				UI.RootTabs.Priority:UpdateWarningBox({
					Title = warningTitle,
					Text = warningBody,
					IsNormal = P.Enabled,
					Visible = true,
	...
	for index, task in ipairs(P.Catalog) do
		task.SeaSet = {};
		for _, sea in ipairs(task.Seas or {}) do
			task.SeaSet[sea] = true;
		end;
		task.Rank = index;
		task.State = "off";
		P.Tasks[task.Id] = task;
		table.insert(P.Order, task.Id);
	end;
	P.ViewSea = 0;
	P.OnlyEnabled = false;
	P.SelectedTask = P.Order[1];
end;
UI.Safe("Priority", function()
	local P = UI.Priority;
	local tab = UI.RootTabs.Priority;
	local defaultCollapsed = UI.Library.DefaultCollapsed;
	UI.Library.DefaultCollapsed = false;
	local function Divider(box, text)
		if not pcall(function()
			box:AddDivider(text);
		end) then
			pcall(function()
	...
	local function PairedButtons(box, first, second)
		local main = box:AddButton(first);
		if not (main and type(main.AddButton) == "function" and pcall(main.AddButton, main, second)) then
			box:AddButton(second);
		end;
		return main;
	end;
	local engine = tab:AddLeftGroupbox("Priority Engine", "list-ordered");
	local order = tab:AddRightGroupbox("Priority Order", "arrow-down-up");
	local sea = tab:AddLeftGroupbox("Sea Rules", "waves");
	local presets = tab:AddRightGroupbox("Presets", "wand-sparkles");
	UI.Library.DefaultCollapsed = defaultCollapsed;

	engine:AddLabel({ DoesWrap = true, Text = "The script normally runs every feature you switch on at the same time. With the engine on it runs the highest job on the list and holds the rest until that one is finished or is switched off." });
	engine:AddToggle("BF_Priority_Enabled", {
		Text = "Enable Priority System",
		Tooltip = "Run the top of the list first and hold everything below it",
		Default = false,
		Callback = function(state)
			P.Enabled = state == true;
			if not P.Enabled then
				P.ReleaseAll();
			end;
			P.Pulse();
	...
	engine:AddSlider("BF_Priority_Slots", {
		Text = "Jobs At Once",
		Tooltip = "How many jobs from the top of the list may run together",
		Min = 1,
		Max = 6,
		Default = 1,
		Rounding = 0,
		Callback = function(value)
			P.Slots = math.max(1, math.floor(tonumber(value) or 1));
			P.Pulse();
		end,
	});
	engine:AddSlider("BF_Priority_Interval", {
		Text = "Check Every",
		Tooltip = "How often the queue is re-checked",
		Min = .2,
		Max = 5,
		Default = .5,
		Rounding = 1,
		Suffix = "s",
		Callback = function(value)
			P.Interval = math.clamp(tonumber(value) or .5, .2, 5);
		end,
	});
	Divider(engine, "Interrupts");
	engine:AddToggle("BF_Priority_LowHealth", {
		Text = "Hold Everything On Low HP",
		Tooltip = "Pause every managed job while your health is below the limit",
		Default = false,
		Callback = function(state)
			P.LowHealthPause = state == true;
			P.Pulse();
		end,
	});
	engine:AddSlider("BF_Priority_HealthPct", {
		Text = "Low HP Limit",
		Tooltip = "Health percentage that counts as low",
		Min = 5,
		Max = 90,
		Default = 25,
		Rounding = 0,
		Suffix = "%",
		Callback = function(value)
			P.HealthPct = math.clamp(tonumber(value) or 25, 5, 90);
			P.Pulse();
		end,
	});
	Divider(engine, "Status");
	P.Controls.ActiveLabel = engine:AddLabel({ DoesWrap = true, Text = "Running: nothing" });
	P.Controls.QueuedLabel = engine:AddLabel({ DoesWrap = true, Text = "Waiting: nothing" });

	local taskNames = {};
	local nameToId = {};
	for _, entry in ipairs(P.Catalog) do
		table.insert(taskNames, entry.Name);
		nameToId[entry.Name] = entry.Id;
	end;
	P.Controls.OrderLabel = order:AddLabel({ DoesWrap = true, Text = "Loading priority list..." });
	order:AddToggle("BF_Priority_OnlyEnabled", {
		Text = "Only Show Switched On",
		Tooltip = "Hide the tasks you have not turned on, handy on a small screen",
		Default = false,
		Callback = function(state)
			P.OnlyEnabled = state == true;
			P.LastOrderText = nil;
			P.Pulse();
		end,
	});
	Divider(order, "Reorder");
	P.Controls.Selector = order:AddDropdown("BF_Priority_Select", {
		Text = "Task",
		Tooltip = "Pick the task you want to move",
		Values = taskNames,
		Default = taskNames[1],
		Multi = false,
		Searchable = true,
		Callback = function(value)
			P.SelectedTask = nameToId[value] or P.SelectedTask;
			P.Pulse();
		end,
	});
	PairedButtons(order, {
		Text = "Move Up",
		Tooltip = "Run this task sooner",
		Func = function()
			P.Move(P.SelectedTask, -1);
			P.Pulse();
		end,
	}, {
		Text = "Move Down",
		Tooltip = "Run this task later",
		Func = function()
			P.Move(P.SelectedTask, 1);
			P.Pulse();
		end,
	...
	PairedButtons(order, {
		Text = "Send To Top",
		Tooltip = "Make this the first thing the script does",
		Func = function()
			P.MoveTo(P.SelectedTask, 1);
			P.Pulse();
		end,
	}, {
		Text = "Send To Bottom",
		Tooltip = "Make this the last thing the script does",
		Func = function()
			P.MoveTo(P.SelectedTask, #P.Order);
			P.Pulse();
		end,
	});
	P.Controls.OrderInput = order:AddInput("BF_Priority_Order", {
		Text = "Saved Order",
		Placeholder = "task ids, comma separated",
		Default = table.concat(P.Order, ","),
		Numeric = false,
		Finished = true,
		Tooltip = "Your order is stored here so configs keep it",
		Callback = function(value)
			if P.SuppressOrderInput then
				return;
			end;
			local ids = {};
			for id in string.gmatch(tostring(value or ""), "[^,%s]+") do
				table.insert(ids, id);
			end;
			P.SetOrder(ids, true);
			P.Pulse();
	...
	P.Controls.SeaLabel = sea:AddLabel({ DoesWrap = true, Text = "Checking sea..." });
	sea:AddLabel({ DoesWrap = true, Text = "Some jobs only exist in one sea. Leviathan is Third Sea only, Sea Beasts spawn in the Second and Third Sea, Pirate Raid is Third Sea and Factory Raid is Second Sea." });
	sea:AddToggle("BF_Priority_SeaLock", {
		Text = "Skip Wrong-Sea Tasks",
		Tooltip = "Jump over jobs that cannot run in the sea you are in",
		Default = true,
		Callback = function(state)
			P.SeaLock = state ~= false;
			P.Pulse();
		end,
	});
	sea:AddDropdown("BF_Priority_ViewSea", {
		Text = "List Filter",
		Tooltip = "Only show tasks that work in the chosen sea",
		Values = { "All Seas", "First Sea", "Second Sea", "Third Sea" },
		Default = "All Seas",
		Multi = false,
		Callback = function(value)
			local map = { ["All Seas"] = 0, ["First Sea"] = 1, ["Second Sea"] = 2, ["Third Sea"] = 3 };
			P.ViewSea = map[value] or 0;
			P.LastOrderText = nil;
			P.Pulse();
		end,
	});
	P.Controls.WrongSeaLabel = sea:AddLabel({ DoesWrap = true, Text = "Checking tasks..." });

	presets:AddLabel({ DoesWrap = true, Text = "Presets rewrite the whole list. Anything a preset does not name keeps its normal place underneath." });
	presets:AddDropdown("BF_Priority_Preset", {
		Text = "Preset",
		Tooltip = "Pick a ready made order",
		Values = P.PresetNames,
		Default = "Default",
		Multi = false,
		Callback = function(value)
			P.SelectedPreset = value;
		end,
	});
	presets:AddButton({
		Text = "Apply Preset",
		Tooltip = "Load the chosen order",
		Func = function()
			P.ApplyPreset(P.SelectedPreset or "Default");
			P.Pulse();
		end,
	});
	presets:AddButton({
		Text = "Reset Order",
		Tooltip = "Put the list back the way it started",
		Func = function()
			P.SetOrder({});
			UI.Library:Notify("Priority order reset.", 3);
			P.Pulse();
		end,
	});
	Divider(presets, "Legend");
	presets:AddLabel({ DoesWrap = true, Text = '<font color="rgb(90, 235, 130)">#</font> running now\n<font color="rgb(255, 205, 90)">+</font> switched on, waiting its turn\n<font color="rgb(255, 110, 110)">x</font> wrong sea, skipped\n<font color="rgb(150, 150, 160)">.</font> switched off' });

	local hooked = {};
	for _, entry in ipairs(P.Catalog) do
		local toggle = entry.Toggle and UI.Toggles[entry.Toggle];
		if toggle and not hooked[entry.Toggle] and type(toggle.OnChanged) == "function" then
			hooked[entry.Toggle] = true;
			pcall(toggle.OnChanged, toggle, function()
				if P.Ready then
					task.spawn(P.Pulse);
				end;
			end);
	...
	P.Pulse();
	task.spawn(function()
		while not UI.Stopped and task.wait(P.Interval) do
			if UI.Library.Unloaded then
				return;
			end;
			P.Pulse();
...
task.spawn(function()
	...
	local function mark(name, ok)
		table.insert(ok and loaded or missing, name);
	end;
	mark("Priority tab", UI.RootTabs.Priority ~= nil and UI.Priority ~= nil and UI.Priority.Ready == true);
	mark("Better Farm", UI.SmartFarm ~= nil and UI.SmartFarm.WrappedCheckQuest == true);
	mark("Money Farm", UI.MoneyFarm ~= nil and UI.MoneyFarm.PlanLabel ~= nil);
	if #missing == 0 then
		UI.Library:Notify("Lumin+ ready: " .. table.concat(loaded, ", "), 8);
	else
		UI.Library:Notify("Lumin+ problem, missing: " .. table.concat(missing, ", "), 12);
	end;
end);
UI.Library:OnUnload(function()
	...
	UI.Stopped = true;
	if UI.Priority and type(UI.Priority.ReleaseAll) == "function" then
		UI.Priority.Ready = false;
		UI.Priority.Enabled = false;
		pcall(UI.Priority.ReleaseAll);
	end;
...
do
	...
	task.delay(3, function()
		...
		local t = game:GetService("TweenService"):Create(bg, tweenInfo, { BackgroundTransparency = 1 });
		local t2 = game:GetService("TweenService"):Create(lbl, tweenInfo, { TextTransparency = 1 });
		t:Play(); t2:Play();
		t.Completed:Wait();
		sg:Destroy();
	end);
...
local function BFCleanupEffects()
	local char = d.Character;
	if not char then return; end;
	for _, desc in ipairs(char:GetDescendants()) do
		if desc:IsA("ParticleEmitter") or desc:IsA("Smoke") or desc:IsA("Sparkles") then
			desc.Enabled = false;
...
getgenv().BFCleanupEffects = BFCleanupEffects;
...
_G.AutoM1Fruit = _G.AutoM1Fruit or false;
local BFAutoM1NextAt = 0;
task.spawn(function()
	while not UI.Stopped do
		task.wait(.05);
		if _G.AutoM1Fruit then
			local now = os.clock();
			if now < BFAutoM1NextAt then continue; end;
			local char = d.Character;
			...
			-- Only fire when an enemy is close enough to actually hit
			local inRange = false;
			for _, mob in ipairs(workspace.Enemies:GetChildren()) do
				local mhum = mob:FindFirstChild("Humanoid");
				local mhrp = mob:FindFirstChild("HumanoidRootPart");
				if mhum and mhrp and mhum.Health > 0 then
					if (mhrp.Position - root.Position).Magnitude <= 60 then
						inRange = true;
						break;
					end;
				end;
			...
			if inRange then
				...
				task.wait(.06);
				K:SendMouseButtonEvent(0, 0, 0, false, game, 1);
				BFAutoM1NextAt = os.clock() + .08;
			end;
		end;
	end;
end);

_G.AutoAttackMode = _G.AutoAttackMode or false;
local BFAutoAttackNextAt = 0;
task.spawn(function()
	while not UI.Stopped do
		task.wait(.05);
		if _G.AutoAttackMode then
			local now = os.clock();
			if now < BFAutoAttackNextAt then continue; end;
			local char = d.Character;
			local hum = char and char:FindFirstChildOfClass("Humanoid");
			if not hum or hum.Health <= 0 then continue; end;
			local target = UI.AutoFarmTarget;
			local thum = target and target:FindFirstChild("Humanoid");
			if not target or not thum or thum.Health <= 0 then continue; end;
			EnsureWeapon();
			local tip = EquippedToolTip();
			...
			task.wait(.06);
			K:SendMouseButtonEvent(0, 0, 0, false, game, 1);
			BFAutoAttackNextAt = os.clock() + .35;
		end;
	end;
end);
...
do
	local Sea1Waypoints = {
		{ Name = "StartTown",    CF = CFrame.new(1059, 15, 1550) },
		{ Name = "JungleIsland", CF = CFrame.new(-1598, 35, 153) },
		{ Name = "Pirate1",      CF = CFrame.new(-1141, 4, 3831) },
		{ Name = "Desert",       CF = CFrame.new(894, 5, 4392) },
		{ Name = "SnowIsland",   CF = CFrame.new(1389, 88, -1298) },
		{ Name = "Marine",       CF = CFrame.new(-938, 5, -3678) },
		{ Name = "Skylands",     CF = CFrame.new(-5179, 1019, -3367) },
		{ Name = "Prison",       CF = CFrame.new(67, 42, -2947) },
		{ Name = "Colosseum",    CF = CFrame.new(594, 101, -1244) },
	};
	local BFInstaTpOriginal = getgenv().BFInstaTp;
	getgenv().BFInstaTp = function(target)
		if typeof(target) ~= "CFrame" then
			if BFInstaTpOriginal then return BFInstaTpOriginal(target); end;
			return;
		end;
		local char = d.Character;
		local root = char and char:FindFirstChild("HumanoidRootPart");
		...
		-- Route through nearest Sea 1 waypoint as intermediate hop
		local bestWp, bestScore;
		for _, wp in ipairs(Sea1Waypoints) do
			local d1 = (wp.CF.Position - root.Position).Magnitude;
			local d2 = (target.Position - wp.CF.Position).Magnitude;
			local score = d1 + d2;
		...
		if bestWp and bestScore < dist * 1.4 then
			_tp(bestWp.CF, true);
			task.wait(.6);
		end;
		_tp(target, true);
	...
	-- UI toggle
	local miscTab = UI.Sections and UI.Sections["BF/Combat"] or nil;
	if miscTab and miscTab.AddToggle then
		miscTab:AddToggle("BF_Toggle_FastAttack", {
			Text = "Fast Attack (Semi-Fast M1)",
			Tooltip = "Rapid left-click loop for faster M1 attacks",
			Default = false,
			Callback = function(v) _G.FastAttack = v; end,
		});
		miscTab:AddToggle("BF_Toggle_AutoM1Fruit", {
			Text = "Auto M1 Fruit",
			Tooltip = "Auto left-click fruit when enemy in range",
			Default = false,
			Callback = function(v) _G.AutoM1Fruit = v; end,
		});
		miscTab:AddToggle("BF_Toggle_AutoAttackMode", {
			Text = "Auto Attack Mode",
			Tooltip = "Auto skill + M1 combo on current target",
			Default = false,
			Callback = function(v) _G.AutoAttackMode = v; end,
		});
	end;
end;
...
if World2 then
	local Sea2Waypoints = {
		{ Name = "W2_Start",        CF = CFrame.new(-429, 71, 1836) },
		{ Name = "W2_Colosseum",    CF = CFrame.new(638, 71, 918) },
		{ Name = "W2_MarineFort",   CF = CFrame.new(-2440, 71, -3216) },
		{ Name = "W2_ZombieIsland", CF = CFrame.new(-5497, 47, -795) },
		{ Name = "W2_SnowMtn",      CF = CFrame.new(609, 400, -5372) },
		{ Name = "W2_IceLab",       CF = CFrame.new(-6064, 15, -4902) },
		{ Name = "W2_FireSide",     CF = CFrame.new(-5428, 15, -5299) },
		{ Name = "W2_IceCastle",    CF = CFrame.new(5667, 26, -6486) },
		{ Name = "W2_ForgotIsland", CF = CFrame.new(-3054, 235, -10142) },
	};
	local _origInstaTp2 = getgenv().BFInstaTp;
	getgenv().BFInstaTp = function(target)
		if typeof(target) ~= "CFrame" then
			if _origInstaTp2 then return _origInstaTp2(target); end;
			return;
		end;
		local char = d.Character;
		local root = char and char:FindFirstChild("HumanoidRootPart");
		...
		local bestWp, bestScore;
		for _, wp in ipairs(Sea2Waypoints) do
			local d1 = (wp.CF.Position - root.Position).Magnitude;
			local d2 = (target.Position - wp.CF.Position).Magnitude;
			local score = d1 + d2;
			if not bestScore or score < bestScore then
...
do
	local _origAutoFarmStep = UI.AutoFarmStep;
	UI.AutoFarmStep = function()
		if World2 and _G.Level then
			local selfChar = d.Character;
			local selfRoot = selfChar and selfChar:FindFirstChild("HumanoidRootPart");
			if selfRoot and CFrameQuest and typeof(CFrameQuest) == "CFrame" then
				local distToQuest = (CFrameQuest.Position - selfRoot.Position).Magnitude;
				-- Ship island portal trigger (level 1250-1349, very far)
				if distToQuest > 8000 then
					local myLv = tonumber(BFDataValue("Level")) or 0;
					if myLv >= 1250 and myLv <= 1349 then
						BFComm("requestEntrance", Vector3.new(923.21252441406, 126.9760055542, 32852.83203125));
						task.wait(.5);
					end;
				end;
		...
		return _origAutoFarmStep();
	end;
end;

do
	local _origCheckHasQuest = CheckHasQuest;
	local BFQuestRetryAt = 0;
	CheckHasQuest = function(Y)
		local result = _origCheckHasQuest(Y);
		if not result and World2 and _G.Level then
			local now = os.clock();
			if now >= BFQuestRetryAt then
				BFQuestRetryAt = now + 3;
				-- Re-assert BFMoveNear to nudge the NPC into range
				if CFrameQuest and typeof(CFrameQuest) == "CFrame" then
					local selfChar = d.Character;
					local selfRoot = selfChar and selfChar:FindFirstChild("HumanoidRootPart");
					if selfRoot and (CFrameQuest.Position - selfRoot.Position).Magnitude < 20 then
						notween(CFrameQuest * CFrame.new(0, 0, -2));
						task.wait(.1);
						notween(CFrameQuest);
					end;
				end;
...
do
	local _origBringEnemy = BringEnemy;
	BringEnemy = function()
		if World2 and _G.Level then
			local myLv = tonumber(BFDataValue("Level")) or 0;
			-- Factory area: interior map, reduce radius to avoid pulling outside walls
			if myLv >= 800 and myLv <= 874 then
				if not _B or typeof(PosMon) ~= "Vector3" then return; end;
				local now = os.clock();
				if now < BFBringNextAt then return; end;
				BFBringNextAt = now + .05;
				for _, R in pairs(workspace.Enemies:GetChildren()) do
					local humanoid = R:FindFirstChild("Humanoid");
					local primary = R.PrimaryPart or R:FindFirstChild("HumanoidRootPart");
					if humanoid and primary and humanoid.Health > 0 then
						if (primary.Position - PosMon).Magnitude <= 150 then
							primary.CFrame = CFrame.new(PosMon);
							humanoid.WalkSpeed = 0;
							humanoid.JumpPower = 0;
						end;
...
do
	local _origAutoFarmStepInner = UI.AutoFarmStep;
	UI.AutoFarmNextQuestRefreshAt2 = 0;
	-- Patch: when World2 and level active, refresh quest data every 0.5s instead of 1s
	local _patchedStep = UI.AutoFarmStep;
	UI.AutoFarmStep = function()
		if World2 and _G.Level then
			local now = os.clock();
			if now >= UI.AutoFarmNextQuestRefreshAt2 then
				UI.AutoFarmNextQuestRefreshAt2 = now + .5;
				pcall(CheckQuest);
			end;
		end;
		return _patchedStep();
...
if World3 then
	local Sea3Waypoints = {
		{ Name = "W3_PiratePort",   CF = CFrame.new(-290, 42, 5581) },
		{ Name = "W3_DragonCrew",   CF = CFrame.new(6738, 127, -713) },
		{ Name = "W3_VenomCrew",    CF = CFrame.new(5213, 1004, 758) },
		{ Name = "W3_MarineTree",   CF = CFrame.new(2180, 27, -6741) },
		{ Name = "W3_Fishman",      CF = CFrame.new(-10581, 330, -8761) },
		{ Name = "W3_DeepForest",   CF = CFrame.new(-13234, 331, -7625) },
		{ Name = "W3_DeepForest2",  CF = CFrame.new(-12680, 389, -9902) },
		{ Name = "W3_Haunted",      CF = CFrame.new(-9479, 141, 5566) },
		{ Name = "W3_NutIsland",    CF = CFrame.new(-2104, 38, -10194) },
		{ Name = "W3_IceCream",     CF = CFrame.new(-820, 65, -10965) },
		{ Name = "W3_Cake",         CF = CFrame.new(-2021, 37, -12028) },
		{ Name = "W3_Chocolate",    CF = CFrame.new(233, 29, -12201) },
		{ Name = "W3_Candy",        CF = CFrame.new(-1150, 20, -14446) },
		{ Name = "W3_TikiOutpost",  CF = CFrame.new(-16547, 61, -173) },
		{ Name = "W3_SubDock",      CF = CFrame.new(-16269, 25, 1373) },
	};
	local _origInstaTp3 = getgenv().BFInstaTp;
	getgenv().BFInstaTp = function(target)
		if typeof(target) ~= "CFrame" then
			if _origInstaTp3 then return _origInstaTp3(target); end;
			return;
		end;
		local char = d.Character;
		local root = char and char:FindFirstChild("HumanoidRootPart");
		...
		local isSubmerged = tPos.Y < -1500;
		if isSubmerged then
			local subDockCF = CFrame.new(-16269, 25, 1373);
			local dockDist = (subDockCF.Position - root.Position).Magnitude;
			if dockDist > 800 then
				local bestWp, bestScore;
				for _, wp in ipairs(Sea3Waypoints) do
					if wp.Name ~= "W3_SubDock" then
						local d1 = (wp.CF.Position - root.Position).Magnitude;
						local d2 = (subDockCF.Position - wp.CF.Position).Magnitude;
						local score = d1 + d2;
						if not bestScore or score < bestScore then
							bestWp = wp;
							bestScore = score;
				...
				if bestWp and bestScore < dockDist * 1.4 then
					_tp(bestWp.CF, true);
					task.wait(.6);
				end;
			end;
			_tp(subDockCF, true);
			task.wait(.5);
			local npc, npcRoot = BFFindNpcLike("submarine");
			if npcRoot then
				...
				for _ = 1, 25 do
					task.wait(.15);
					local loc = tostring(d:GetAttribute("CurrentLocation") or "");
					if loc == "Submerged Island" or loc == "Sealed Cavern" then break; end;
				end;
			end;
			local newChar = d.Character;
			local newRoot = newChar and newChar:FindFirstChild("HumanoidRootPart");
			if newRoot then newRoot.CFrame = target; end;
			return;
		end;
		local bestWp, bestScore;
		for _, wp in ipairs(Sea3Waypoints) do
...
do
	local _origAutoFarmStepS3 = UI.AutoFarmStep;
	UI.AutoFarmNextQuestRefreshAtS3 = 0;
	UI.AutoFarmStep = function()
		if World3 and _G.Level then
			local selfChar = d.Character;
			local selfRoot = selfChar and selfChar:FindFirstChild("HumanoidRootPart");
			if selfRoot and CFrameQuest and typeof(CFrameQuest) == "CFrame" then
				local now = os.clock();
				local myLv = tonumber(BFDataValue("Level")) or 0;
				local distToQuest = (CFrameQuest.Position - selfRoot.Position).Magnitude;
				if myLv >= 2450 and distToQuest > 12000 then
					local tikiFront = Vector3.new(-16270, 25, 1373);
					BFComm("requestEntrance", tikiFront);
					task.wait(.6);
				end;
				if now >= UI.AutoFarmNextQuestRefreshAtS3 then
					UI.AutoFarmNextQuestRefreshAtS3 = now + .5;
					pcall(CheckQuest);
				end;
			end;
		end;
		return _origAutoFarmStepS3();
	end;
end;

do
	local _origCheckHasQuestS3 = CheckHasQuest;
	local BFQuestRetryS3At = 0;
	CheckHasQuest = function(Y)
		local result = _origCheckHasQuestS3(Y);
		if not result and World3 and _G.Level then
			local now = os.clock();
			if now >= BFQuestRetryS3At then
				BFQuestRetryS3At = now + 3;
				if CFrameQuest and typeof(CFrameQuest) == "CFrame" then
					local selfChar = d.Character;
					local selfRoot = selfChar and selfChar:FindFirstChild("HumanoidRootPart");
					if selfRoot and (CFrameQuest.Position - selfRoot.Position).Magnitude < 20 then
...
do
	local _origBringEnemyS3 = BringEnemy;
	BringEnemy = function()
		if World3 and _G.Level then
			local myLv = tonumber(BFDataValue("Level")) or 0;
			if myLv >= 2025 and myLv <= 2074 then
				if not _B or typeof(PosMon) ~= "Vector3" then return; end;
				local now = os.clock();
				if now < BFBringNextAt then return; end;
				BFBringNextAt = now + .05;
				for _, R in pairs(workspace.Enemies:GetChildren()) do
					...
					local primary = R.PrimaryPart or R:FindFirstChild("HumanoidRootPart");
					if humanoid and primary and humanoid.Health > 0 then
						if (primary.Position - PosMon).Magnitude <= 180 then
							primary.CFrame = CFrame.new(PosMon);
							humanoid.WalkSpeed = 0;
							humanoid.JumpPower = 0;
						end;
...
do
	local _origDragonTalonStep = UI.DragonTalonStep;
	UI.DragonTalonStep = function(active)
		if active and World3 then
			local status = _origDragonTalonStep(active);
			if status == "buying-dragon-talon" then
				local _, npcRoot = BFFindNpcLike("longma");
				local npcTarget = npcRoot or CFrame.new(-9632, 143, 5910);
				if not BFMoveNear(npcTarget, 10) then
					_tp(typeof(npcTarget) == "CFrame" and npcTarget or CFrame.new(npcRoot.Position), true);
					task.wait(.5);
				end;
			end;
			return status;
		end;
		return _origDragonTalonStep(active);
	end;
end;

do
	local _origGodhumanStep = UI.GodhumanStep;
	UI.GodhumanStep = function(active)
		if active and World3 then
			local status = _origGodhumanStep(active);
			if status == "buying-godhuman" then
				local _, npcRoot = BFFindNpcLike("strongest");
				local npcTarget = npcRoot or CFrame.new(5188, 1004, 748);
				if not BFMoveNear(npcTarget, 10) then
					_tp(typeof(npcTarget) == "CFrame" and npcTarget or CFrame.new(npcRoot.Position), true);
					task.wait(.5);
				end;
		...
		return _origGodhumanStep(active);
	end;
end;
...
do
	...
	local S3_ZoneMap      = {
		["Lv 1"]        = CFrame.new(-21998, 30.0, -682),
		["Lv 2"]        = CFrame.new(-26779, 30.0, -822),
		["Lv 3"]        = CFrame.new(-31171, 30.0, -2256),
		["Lv 4"]        = CFrame.new(-34054, 30.2, -2560),
		["Lv 5"]        = CFrame.new(-38887, 30.0, -2162),
		["Lv 6"]        = CFrame.new(-44541, 30.0, -1244),
		["Lv Infinite"] = CFrame.new(-50000,  31,  37016),
	};
	...
	task.spawn(function()
		while not UI.Stopped do
			task.wait(.5);
			pcall(function()
				if not World3 or not _G.SailBoats then return; end;
				if CheckBoat() then return; end;
				local humanoid = BFHumanoid();
				...
				S3_BuyLastAt = now;
				UI.BoatPurchaseAt = now;
				BFComm("BuyBoat", _G.SelectedBoat or "Guardian");
				for _ = 1, 25 do
					task.wait(.3);
	...
	if _G.SailBoat_Hydra ~= nil then
		task.spawn(function()
			while not UI.Stopped do
				task.wait(.05);
				pcall(function()
					if not World3 or not _G.SailBoat_Hydra then return; end;
					local humanoid = BFHumanoid();
					if not humanoid or not humanoid.Sit then return; end;
					if S3_HasEnemy() then return; end;
					local boat = CheckBoat();
...
do
	if World3 then
		...
		local function S3_AtkBurst(target)
			if not target or not target.Parent then return; end;
			local root = target:FindFirstChild("HumanoidRootPart") or (target:IsA("BasePart") and target);
			if not root then return; end;
			MousePos = root.Position;
			if CheckF() then
				weaponSc("Blox Fruit");
		...
		local function S3_EnemyHP(model)
			local h = model and model:FindFirstChild("Health");
			return h and (tonumber(h.Value) or 0) or 0;
		end;
			...
			while not UI.Stopped do
				...
				pcall(function()
					...
					for _, e in ipairs(enemies:GetChildren()) do
						...
						if e.Name == "Shark" and _G.Shark then
							_tp(root.CFrame * CFrame.new(0, 12, 8));
							S3_AtkBurst(e);
							repeat task.wait(.05); until not _G.Shark or not e.Parent or h.Health <= 0;
						elseif e.Name == "Terrorshark" and _G.TerrorShark then
							_tp(root.CFrame * CFrame.new(0, 12, 8));
							S3_AtkBurst(e);
							repeat task.wait(.05); until not _G.TerrorShark or not e.Parent or h.Health <= 0;
						elseif e.Name == "Piranha" and _G.Piranha then
							_tp(root.CFrame * CFrame.new(0, 8, 5));
							S3_AtkBurst(e);
							repeat task.wait(.05); until not _G.Piranha or not e.Parent or h.Health <= 0;
						end;
					end;
				end);
			end;
			...
			while not UI.Stopped do
				...
				pcall(function()
					...
					for _, e in ipairs(enemies:GetChildren()) do
						...
						local root = e:FindFirstChild("HumanoidRootPart");
						if not root then continue; end;
						if e.Name == "Fish Crew Member" and _G.MobCrew then
							_tp(root.CFrame * CFrame.new(0, 10, 6));
							S3_AtkBurst(e);
							repeat task.wait(.05); until not _G.MobCrew or not e.Parent or h.Health <= 0;
						elseif e.Name == "Haunted Crew Member" and _G.HCM then
							_tp(root.CFrame * CFrame.new(0, 10, 6));
							S3_AtkBurst(e);
							repeat task.wait(.05); until not _G.HCM or not e.Parent or h.Health <= 0;
						end;
					end;
				end);
			end;
		...
		task.spawn(function()
			while not UI.Stopped do
				task.wait(.05);
				pcall(function()
					if not World3 or not _G.FishBoat then return; end;
					local enemies = workspace:FindFirstChild("Enemies");
					if not enemies then return; end;
					for _, R in ipairs(enemies:GetChildren()) do
						if R.Name ~= "FishBoat" then continue; end;
						local hp  = R:FindFirstChild("Health");
						local eng = BFFirstPart(R:FindFirstChild("Engine"));
						if not hp or not eng or (tonumber(hp.Value) or 0) <= 0 then continue; end;
						local waterY = S3_Water();
						repeat
							task.wait(.05);
							eng = BFFirstPart(R:FindFirstChild("Engine"));
							if not eng then break; end;
							_tp(CFrame.new(eng.Position.X, waterY + 80, eng.Position.Z - 20));
							MousePos = eng.Position;
							S3_AtkBurst(eng);
						until not _G.FishBoat or not R.Parent or not hp.Parent or (tonumber(hp.Value) or 0) <= 0;
					end;
				end);
			end;
		end);
		...
		task.spawn(function()
			while not UI.Stopped do
				task.wait(.05);
				pcall(function()
					if not World3 or not _G.PGB then return; end;
					local enemies = workspace:FindFirstChild("Enemies");
					if not enemies then return; end;
					for _, R in ipairs(enemies:GetChildren()) do
						if R.Name ~= "PirateGrandBrigade" and R.Name ~= "PirateBrigade" then continue; end;
						local hp  = R:FindFirstChild("Health");
						local eng = BFFirstPart(R:FindFirstChild("Engine"));
						if not hp or not eng or (tonumber(hp.Value) or 0) <= 0 then continue; end;
						local offset = R.Name == "PirateBrigade"
							and CFrame.new(0, -25, -8)
							or  CFrame.new(0, -40, -45);
						local waterY = S3_Water();
						repeat
							task.wait(.05);
							eng = BFFirstPart(R:FindFirstChild("Engine"));
							if not eng then break; end;
							_tp(CFrame.new(eng.Position.X, waterY + 90, eng.Position.Z - 30));
							MousePos = eng.Position;
							S3_AtkBurst(eng);
						until not _G.PGB or not R.Parent or not hp.Parent or (tonumber(hp.Value) or 0) <= 0;
					end;
				end);
			end;
		end);
		...
		task.spawn(function()
			while not UI.Stopped do
				task.wait(.05);
				pcall(function()
					if not World3 or not _G.SeaBeast1 then return; end;
					local beasts = workspace:FindFirstChild("SeaBeasts");
					if not beasts then return; end;
					for _, beast in ipairs(beasts:GetChildren()) do
						local root   = beast:FindFirstChild("HumanoidRootPart");
						local health = beast:FindFirstChild("Health");
						if not root or not health or (tonumber(health.Value) or 0) <= 0 then continue; end;
						local waterY = S3_Water();
						repeat
							task.wait(.05);
							root = beast:FindFirstChild("HumanoidRootPart");
							health = beast:FindFirstChild("Health");
							if not root or not health then break; end;
							local atkY = waterY + 160;
							_tp(CFrame.new(root.Position.X, atkY, root.Position.Z));
							MousePos = root.Position;
							S3_AtkBurst(root);
						until not _G.SeaBeast1 or not beast.Parent or not health.Parent or (tonumber(health.Value) or 0) <= 0;
					end;
				end);
			end;
		end);
		...
		task.spawn(function()
			while not UI.Stopped do
				task.wait(.05);
				pcall(function()
					if not World3 or not _G.Leviathan1 then return; end;
					local beasts = workspace:FindFirstChild("SeaBeasts");
					if not beasts then return; end;
					local lev = beasts:FindFirstChild("Leviathan");
					if not lev then return; end;
					local health  = lev:FindFirstChild("Health");
					local segment = lev:FindFirstChild("Leviathan Segment");
					local root    = lev:FindFirstChild("HumanoidRootPart");
					if not health or (tonumber(health.Value) or 0) <= 0 then return; end;
					local atkTarget = segment or root;
					if not atkTarget then return; end;
					local waterY = S3_Water();
					repeat
						task.wait(.05);
						health  = lev:FindFirstChild("Health");
						segment = lev:FindFirstChild("Leviathan Segment");
						root    = lev:FindFirstChild("HumanoidRootPart");
						if not health or not lev.Parent then break; end;
						atkTarget = segment or root;
						if not atkTarget then break; end;
						local tPos = atkTarget:IsA("BasePart") and atkTarget.Position or atkTarget:GetPivot().Position;
						_tp(CFrame.new(tPos.X, waterY + 220, tPos.Z));
						MousePos = tPos;
						S3_AtkBurst(atkTarget);
					until not _G.Leviathan1 or not lev.Parent or not health.Parent or (tonumber(health.Value) or 0) <= 0;
				end);
			end;
		end);

		task.spawn(function()
			local KIT_TARGET = CFrame.new(-10000000, 31, 37016.25);
			while not UI.Stopped do
				task.wait(.05);
				pcall(function()
					if not World3 or not _G.AutofindKitIs then return; end;
					if BFWorldLocation("Kitsune Island") then return; end;
					local humanoid = BFHumanoid();
					if not humanoid or not humanoid.Sit then return; end;
					local boat = CheckBoat();
					if not boat then return; end;
					...
					if not seat then return; end;
					if CheckEnemiesBoat() or CheckTerrorShark() or CheckPirateGrandBrigade() then
						seat.CFrame = KIT_TARGET * CFrame.new(0, 150, 0);
					else
						seat.CFrame = KIT_TARGET;
					end;
				end);
			end;
		end);
		...
		task.spawn(function()
			while not UI.Stopped do
				...
				pcall(function()
					if not World3 then return; end;
					if not (_G.Prehis_Find or _G.FindMirage) then return; end;
					local humanoid = BFHumanoid();
					if not humanoid or not humanoid.Sit then return; end;
					local boat = CheckBoat();
					if not boat then return; end;
					local seat = BFBoatSeat(boat);
					if not seat then return; end;
					local destCF = _G.FindMirage
						and CFrame.new(-10000000, 31, 37016.25)
						or  CFrame.new(-10000000, 31, 37016.25);
					if CheckEnemiesBoat() or CheckTerrorShark() or CheckPirateGrandBrigade() then
						seat.CFrame = destCF * CFrame.new(0, 150, 0);
					else
						seat.CFrame = destCF;
					end;
				end);
		...
		task.spawn(function()
			...
			while not UI.Stopped do
				task.wait(.5);
				pcall(function()
					if not World3 or not _G.SailBoats then
						hadBoat = false;
						return;
					end;
					local boat = CheckBoat();
					...
					elseif hadBoat then
						hadBoat = false;
						local now = os.clock();
						if now - S3_RebuyAt < 3 then return; end;
						S3_RebuyAt = now;
						local char = d.Character;
						local root = char and char:FindFirstChild("HumanoidRootPart");
						if not root then return; end;
						notween(CFrame.new(-16927.451, 9.086, 433.864));
						task.wait(.4);
						UI.BoatPurchaseAt = now;
						BFComm("BuyBoat", _G.SelectedBoat or "Guardian");
					end;
...
if UI and UI.SmartFarm and UI.MoneyFarm then
do
	local S  = UI.SmartFarm;
	local M  = UI.MoneyFarm;
	local CS = game:GetService("CollectionService");

	local function DataNum(name)
		return tonumber(BFDataValue(name)) or 0;
	end;
	...
	local function LocationName()
		local ok, val = pcall(function()
			return g and g:GetAttribute("CurrentLocation");
		end);
		if ok and val then return tostring(val); end;
		local ok2, val2 = pcall(function()
			return d and d:GetAttribute("CurrentLocation");
		end);
		if ok2 and val2 then return tostring(val2); end;
		return "";
	end;

	local function WorldKey()
		return LocationName() .. "|" .. tostring(game.PlaceId);
	end;

	local GoToLast, GoToLastAt = nil, 0;
	local function GoTo(cf)
		...
		local now = os.clock();
		local same = GoToLast and (GoToLast.Position - cf.Position).Magnitude < 6;
		if same and now < GoToLastAt + 1.5 then return; end;
		GoToLast, GoToLastAt = cf, now;
		_tp(cf, true);
	end;

	local function Reached(pos, tol)
		local root = SelfRoot();
		if not root then return false; end;
		return (root.Position - pos).Magnitude <= (tonumber(tol) or 8);
	end;
	...
	S.ChoiceAt       = 0;
	S.LastExpSeen    = 0;
	S.LastExpMoveAt  = os.clock();

	function S.InvalidateWorld()
		S.SpawnCache    = {};
		S.SpawnMeta     = {};
		S.SpawnMetaAt   = 0;
		S.ClusterCache  = {};
	...
	task.spawn(function()
		while not UI.Stopped and task.wait(2) do
			local key = WorldKey();
			if key ~= S.WorldKey then
				S.WorldKey = key;
				pcall(S.InvalidateWorld);
			end;
		end;
	end);

	function S.ScanSpawns()
		local now = os.clock();
		if S.SpawnMeta and next(S.SpawnMeta) ~= nil and now < S.SpawnMetaAt + 20 then
			return S.SpawnMeta;
		end;
		local origin = workspace:FindFirstChild("_WorldOrigin");
		local holder = origin and origin:FindFirstChild("EnemySpawns");
			...
			for _, part in ipairs(holder:GetChildren()) do
				...
				if plain then
					...
					if active ~= false then
						...
						if pos then
							local key = string.lower(plain);
							local bucket = meta[key];
							if not bucket then
								bucket = { Name = plain, Points = {}, Level = 0, Boss = string.find(part.Name, "%[Boss%]") ~= nil };
								meta[key] = bucket;
							end;
							table.insert(bucket.Points, pos);
							local lv = tonumber(string.match(part.Name, "%[Lv%.%s*(%d+)")) or 0;
							if lv > bucket.Level then bucket.Level = lv; end;
						end;
					end;
				end;
			end;
	...
	function S.MobInfo(mobName)
		if not mobName then return nil; end;
		local meta = S.ScanSpawns();
		return meta[string.lower(mobName)];
	end;

	function S.SpawnPoints(mobName)
		local info = S.MobInfo(mobName);
		return info and info.Points or {};
	end;

	function S.HasSpawns(mobName)
		local info = S.MobInfo(mobName);
		return info ~= nil and #info.Points > 0;
	end;

	function S.MobLevel(mobName)
		local info = S.MobInfo(mobName);
		return info and info.Level or 0;
	end;

	function S.LiveCount(mobName)
		local holder = workspace:FindFirstChild("Enemies");
		if not holder or not mobName then return 0; end;
		local wanted = string.lower(mobName);
		local count = 0;
		for _, enemy in ipairs(holder:GetChildren()) do
			local plain = string.lower(string.match(enemy.Name, "^(.-)%s*%[Lv%.") or enemy.Name);
			if plain == wanted then
				local hum = enemy:FindFirstChildOfClass("Humanoid");
				if hum and hum.Health > 0 then count = count + 1; end;
			end;
		end;
		return count;
	end;
	...
	function S.BestCluster(mobName)
		if not mobName then return nil, 0, 0; end;
		local key    = string.lower(mobName);
		local cached = S.ClusterCache[key];
		if cached and os.clock() < cached.At + 30 and cached.Radius == S.ClusterRadius then
			return cached.Centre, cached.Count, cached.Total;
		end;
		local points = S.SpawnPoints(mobName);
		if #points == 0 then return nil, 0, 0; end;
		local radius = S.ClusterRadius;
		local live   = {};
		local holder = workspace:FindFirstChild("Enemies");
		if holder then
			...
			for _, enemy in ipairs(holder:GetChildren()) do
				...
				if plain == wanted then
					...
					if hum and root and hum.Health > 0 then
						table.insert(live, root.Position);
					end;
				end;
			end;
		end;
		local bestCentre, bestScore, bestCount;
		for _, anchor in ipairs(points) do
			local sum, count = Vector3.new(), 0;
			for _, other in ipairs(points) do
				if (other - anchor).Magnitude <= radius then
			...
			local alive  = 0;
			for _, lp in ipairs(live) do
				if (lp - centre).Magnitude <= radius then alive = alive + 1; end;
			end;
			local score = count + alive * 3;
			if not bestScore or score > bestScore then
				bestScore  = score;
				bestCentre = centre;
				bestCount  = count;
	...
	function S.TierKey(entry)
		return tostring(entry.Id) .. "#" .. tostring(entry.Tier);
	end;

	function S.TierBlocked(entry)
		local until_ = S.TierBlack[S.TierKey(entry)];
		return until_ ~= nil and os.clock() < until_;
	end;

	function S.BlockTier(entry, seconds)
		if not entry then return; end;
		S.TierBlack[S.TierKey(entry)] = os.clock() + (tonumber(seconds) or 300);
	end;

	function S.ScoreEntry(entry, level, from)
		local info = S.MobInfo(entry.Mob);
		if not info or #info.Points == 0 then return nil; end;
		local mobLv = info.Level;
		if mobLv > 0 and level > 0 and mobLv > level * 2.2 + 40 then
			return nil;
		end;
		local centre = select(1, S.BestCluster(entry.Mob));
		if not centre then return nil; end;
		local travel = from and (centre - from).Magnitude or 0;
		local alive  = S.LiveCount(entry.Mob);
		local perKill = entry.PerKill;
		local density = math.clamp(alive, 0, 12);
		local supply  = 1 + density * 0.25 + math.min(#info.Points, 20) * 0.05;
		local reach   = 1 + travel / 4000;
		local risk    = 1;
		if mobLv > 0 and level > 0 and mobLv > level * 1.35 then
			risk = 1 + (mobLv / math.max(level, 1) - 1.35) * 1.6;
		end;
		return (perKill * supply) / (reach * risk);
	end;
	...
	function S.PickQuest(level)
		...
		local root = SelfRoot();
		local from = root and root.Position or nil;
		local best, bestScore;
		for _, entry in ipairs(ladder) do
			if entry.LevelReq <= level - S.MinLevelGap
				and not S.TierBlocked(entry)
				and not (S.SkipBosses and entry.Boss)
				and S.HasSpawns(entry.Mob) then
				if S.Strategy == "Highest XP" then
					local mobLv = S.MobLevel(entry.Mob);
					local safe  = not (mobLv > 0 and level > 0 and mobLv > level * 2.2 + 40);
					if safe then
						if not best
							or entry.LevelReq > best.LevelReq
							or (entry.LevelReq == best.LevelReq and entry.Exp > best.Exp) then
							best = entry;
						end;
					end;
				else
					local score = S.ScoreEntry(entry, level, from);
		...
		if not best then
			for _, entry in ipairs(ladder) do
				if entry.LevelReq <= level and S.HasSpawns(entry.Mob) and not S.TierBlocked(entry) then
					if not best or entry.LevelReq > best.LevelReq then best = entry; end;
				end;
			end;
		end;
		if not best then
			for _, entry in ipairs(ladder) do
				if entry.LevelReq <= level and S.HasSpawns(entry.Mob) then
					if not best or entry.LevelReq > best.LevelReq then best = entry; end;
				end;
			end;
		end;
	...
	local _origBuildLadder = S.BuildLadder;
	function S.BuildLadder(force)
		local ladder = _origBuildLadder(force);
		if ladder and #ladder > 0 then return ladder; end;
		local now = os.clock();
		if S.FallbackLadder and now < (S.FallbackAt or 0) + 60 then
			S.Ladder = S.FallbackLadder;
			return S.Ladder;
		end;
		local meta  = S.ScanSpawns();
		local built = {};
		...
		if holder then
			for _, npc in ipairs(holder:GetChildren()) do
				local root = npc:IsA("Model") and npc:FindFirstChild("HumanoidRootPart");
				if root and string.find(string.lower(npc.Name), "quest giver", 1, true) then
					local label = string.gsub(npc.Name, "%s*Quest Giver.*$", "");
					local nearest, nearestDist;
					for _, info in pairs(meta) do
						if #info.Points > 0 then
							local dist = (info.Points[1] - root.Position).Magnitude;
							if not nearestDist or dist < nearestDist then
								nearest, nearestDist = info, dist;
							end;
						end;
					end;
					if nearest and nearestDist and nearestDist < 3000 then
						table.insert(built, {
							Id       = label .. "Quest",
							Tier     = 1,
							LevelReq = math.max(0, nearest.Level - 5),
							Title    = label,
							Mob      = nearest.Name,
							Kills    = 10,
							Distinct = 1,
							Exp      = nearest.Level * 120,
							Beli     = nearest.Level * 12,
							PerKill  = nearest.Level * 12,
							Boss     = nearest.Boss == true,
						});
					end;
				end;
			end;
		end;
		table.sort(built, function(a, b) return a.LevelReq < b.LevelReq; end);
		S.FallbackLadder = built;
		S.FallbackAt     = now;
		S.Ladder         = built;
		return built;
	end;

	function S.GiverName(questId)
		local base = string.gsub(tostring(questId), "Quest%d*$", "");
		base = string.gsub(base, "(%l)(%u)", "%1 %2");
		return string.lower(base);
	end;

	function S.FindGiver(questId, near)
		local strikes = S.GiverStrikes[questId] or 0;
		if strikes < 3 then
			local cached = S.CachedGiver(questId);
			if cached then return cached, "cached"; end;
		else
			S.NpcCache[questId] = nil;
			pcall(S.SaveCache);
		end;
		local holder = workspace:FindFirstChild("NPCs");
		if not holder then return nil, "no-npc-folder"; end;
		local wanted = S.GiverName(questId);
		local tokens = {};
		...
		for _, npc in ipairs(holder:GetChildren()) do
			local root = npc:IsA("Model") and npc:FindFirstChild("HumanoidRootPart");
			if root and string.find(string.lower(npc.Name), "quest giver", 1, true) then
				local label = string.lower(string.gsub(npc.Name, "%s*Quest Giver.*$", ""));
				local score = 0;
				if label == wanted then
					score = 1000;
				else
					for _, tok in ipairs(tokens) do
						if string.find(label, tok, 1, true) then score = score + 100; end;
					end;
					for word in string.gmatch(label, "%a+") do
						if #word > 2 and string.find(wanted, word, 1, true) then score = score + 60; end;
					end;
				end;
				local dist = near and (root.Position - near).Magnitude or 0;
				if near then score = score - math.min(dist / 100, 40); end;
				if score > 0 and (not bestScore or score > bestScore) then
					best, bestScore = root.Position, score;
					bestWhy = score >= 1000 and "name-match" or "fuzzy-match";
				end;
			end;
		end;
		if best and bestScore and bestScore >= 40 then
			if bestScore >= 100 then S.RememberGiver(questId, best); end;
			return best, bestWhy;
		end;
		if near then
			local fbBest, fbDist;
			for _, npc in ipairs(holder:GetChildren()) do
				local root = npc:IsA("Model") and npc:FindFirstChild("HumanoidRootPart");
				if root and string.find(string.lower(npc.Name), "quest giver", 1, true) then
					local dist = (root.Position - near).Magnitude;
					if not fbDist or dist < fbDist then fbBest, fbDist = root.Position, dist; end;
				end;
			end;
			if fbBest and fbDist and fbDist <= 2500 then return fbBest, "nearest"; end;
		end;
		return nil, "not-found";
	end;

	function S.NoteGiverResult(questId, worked)
		if not questId then return; end;
		if worked then
			S.GiverStrikes[questId] = 0;
		else
			S.GiverStrikes[questId] = (S.GiverStrikes[questId] or 0) + 1;
		end;
	end;

	local _origPreview = S.Preview;
	function S.Preview()
		local level = DataNum("Level");
		local key = table.concat({
			tostring(level),
			tostring(S.Strategy),
			tostring(S.SkipBosses),
			tostring(S.ClusterRadius),
			tostring(S.MinLevelGap),
			S.WorldKey,
		}, "/");
		local now = os.clock();
		if S.ChoiceCache and key == S.ChoiceKey and now < S.ChoiceAt + 5 then
			S.Choice  = S.ChoiceCache;
			S.Blocker = nil;
			return S.ChoiceCache;
		end;
		local choice = _origPreview();
		S.ChoiceKey = key;
		S.ChoiceAt  = now;
		S.ChoiceCache = choice;
		return choice;
	end;

	local _origApply = S.Apply;
	function S.Apply()
		local before = S.Choice;
		local ok = _origApply();
		if ok and S.Choice then
			local exp = DataNum("Exp");
			local lvl = DataNum("Level");
			local moved = exp ~= S.LastExpSeen;
			if moved or lvl ~= (S.LastLevelSeen or lvl) then
				S.LastExpSeen   = exp;
				S.LastLevelSeen = lvl;
				S.LastExpMoveAt = os.clock();
			elseif os.clock() - S.LastExpMoveAt > 120 then
				S.BlockTier(S.Choice, 300);
				S.NoteGiverResult(S.Choice.Id, false);
				S.LastExpMoveAt = os.clock();
				S.ChoiceCache   = nil;
				S.ChoiceKey     = "";
				S.SetStatus("tier-stalled-switching");
				return _origApply();
			end;
			if before and S.Choice and before.Id ~= S.Choice.Id then
				S.LastExpMoveAt = os.clock();
			end;
		end;
		return ok;
	...
	M.ChestCache   = nil;
	M.ChestCacheAt = 0;
	M.ChestBlack   = setmetatable({}, { __mode = "k" });
	M.Route        = {};
	M.RouteAt      = 0;
	M.BossLock     = nil;
	M.BossLockAt   = 0;
	M.BossBlack    = setmetatable({}, { __mode = "k" });
	M.WaitRespawn  = true;
	M.Earn = {
		Chests = { Beli = 0, Time = 0 },
		Bosses = { Beli = 0, Time = 0 },
		Quests = { Beli = 0, Time = 0 },
	};
	M.ActiveSource = nil;
	M.LastEarnBeli = 0;
	...
	function M.TweenTo(target, tolerance)
		...
		if Reached(goal.Position, tonumber(tolerance) or 8) then return true; end;
		GoTo(goal);
		return false;
	end;
	...
	function M.LiveChests()
		local now = os.clock();
		if M.ChestCache and now < M.ChestCacheAt + 0.25 then
			return M.ChestCache;
		end;
		local list = {};
		for _, chest in ipairs(CS:GetTagged("_ChestTagged")) do
			if chest:IsDescendantOf(workspace) and not chest:GetAttribute("IsDisabled") then
				local blocked = M.ChestBlack[chest];
	...
	function M.BuildRoute(from)
		local pool = {};
		for _, chest in ipairs(M.LiveChests()) do
			local ok, piv = pcall(chest.GetPivot, chest);
			if ok then
				table.insert(pool, { Obj = chest, Pos = piv.Position });
			end;
		end;
		local route, cur = {}, from;
		while #pool > 0 do
			local bi, bd;
			for i, entry in ipairs(pool) do
				local dist = (entry.Pos - cur).Magnitude;
				if not bd or dist < bd then bi, bd = i, dist; end;
			end;
			local pick = table.remove(pool, bi);
	...
	function M.NextRouteStop(from, maxDistance)
		local now = os.clock();
		if #M.Route == 0 or now > M.RouteAt + 25 then
			M.BuildRoute(from);
		end;
		while #M.Route > 0 do
			local entry = M.Route[1];
			...
			if not chest or not chest.Parent or chest:GetAttribute("IsDisabled") then
				...
				if blocked and now < blocked then
					...
					local dist = (entry.Pos - from).Magnitude;
					if maxDistance and dist > maxDistance then return nil, dist; end;
					return entry, dist;
				end;
			end;
		end;
	...
	function M.GrabChest(entry)
		local chest = entry and entry.Obj;
		if not chest or not chest.Parent then return true; end;
		local goal = entry.Pos + Vector3.new(0, 3, 0);
		GoTo(CFrame.new(goal));
		if not Reached(goal, 12) then return false; end;
		local deadline = os.clock() + 3;
		local nudgeAt  = 0;
		repeat
			task.wait(.08);
			if not chest.Parent or chest:GetAttribute("IsDisabled") then return true; end;
			if os.clock() >= nudgeAt then
				nudgeAt = os.clock() + .4;
				GoTo(CFrame.new(goal));
			end;
		until os.clock() >= deadline;
		M.ChestBlack[chest] = os.clock() + 25;
		return false;
	end;
	...
	function M.LiveBoss()
		...
		for _, enemy in ipairs(holder:GetChildren()) do
			local plain = string.match(enemy.Name, "^(.-)%s*%[Lv%.") or enemy.Name;
			if bosses[plain] then
				local blocked = M.BossBlack[enemy];
				if not blocked or now >= blocked then
					local humanoid = enemy:FindFirstChild("Humanoid");
					local eroot    = enemy:FindFirstChild("HumanoidRootPart");
					if humanoid and eroot and humanoid.Health > 0 then
						local bossLevel = tonumber(string.match(enemy.Name, "%[Lv%.%s*(%d+)")) or 0;
						if bossLevel <= level * 1.5 then
							local dist  = from and (eroot.Position - from).Magnitude or 0;
							local score = bossLevel * 10 - dist / 50;
							if not bestScore or score > bestScore then
	...
	function M.TickBoss()
		local lock = M.BossLock;
		if not lock then return nil; end;
		if not lock.Parent then
			M.BossLock = nil;
			M.Stats.Bosses = M.Stats.Bosses + 1;
			return nil;
		end;
		local humanoid = lock:FindFirstChild("Humanoid");
		if not humanoid or humanoid.Health <= 0 then
			M.BossLock = nil;
			M.Stats.Bosses = M.Stats.Bosses + 1;
			return nil;
		end;
		if os.clock() - M.BossLockAt > 120 then
			M.BossBlack[lock] = os.clock() + 300;
			M.BossLock = nil;
			return nil;
		end;
		return lock;
	...
	function M.SourceRate(name)
		local slot = M.Earn[name];
		if not slot or slot.Time < 30 then return nil; end;
		return slot.Beli / slot.Time;
	end;
	...
	function M.TrackEarnings()
		...
		M.LastEarnAt   = now;
		M.LastEarnBeli = beli;
		if not M.ActiveSource or dt <= 0 or dt > 5 then return; end;
		local slot = M.Earn[M.ActiveSource];
		if not slot then return; end;
		slot.Time = slot.Time + dt;
		if gain > 0 then slot.Beli = slot.Beli + gain; end;
		if slot.Time > 900 then
			slot.Time = slot.Time * 0.5;
			slot.Beli = slot.Beli * 0.5;
		end;
	end;

	function M.ResetEarnings()
		for _, slot in pairs(M.Earn) do
			slot.Beli = 0;
			slot.Time = 0;
		end;
		M.ActiveSource = nil;
	...
	function M.PickSource(hasChest, hasBoss, hasQuest)
		local options = {};
		if hasChest then table.insert(options, "Chests"); end;
		if hasBoss  then table.insert(options, "Bosses"); end;
		if hasQuest then table.insert(options, "Quests"); end;
		if #options == 0 then return nil; end;
		if #options == 1 then return options[1]; end;
		for _, name in ipairs(options) do
			if M.SourceRate(name) == nil then return name; end;
		end;
		local best, bestRate;
		for _, name in ipairs(options) do
			local rate = M.SourceRate(name) or 0;
			if not bestRate or rate > bestRate then best, bestRate = name, rate; end;
		end;
		return best;
	end;
	...
	function M.Step()
		...
		local root = SelfRoot();
		if not root then return "waiting-for-character"; end;
		local here = root.Position;
		...
		if wantBosses then
			local locked = M.TickBoss();
			if locked then
				M.Target       = locked;
				M.ActiveSource = "Bosses";
				M.Plan         = "Boss: " .. locked.Name;
				f.Kill(locked, _G.MoneyFarm);
				return "fighting-boss";
			end;
		end;

		if wantChests then
			local entry, dist = M.NextRouteStop(here, M.GrabRadius);
			if entry then
				M.Target       = entry.Obj;
				M.ActiveSource = "Chests";
				M.Plan         = string.format("Chest %d studs away", math.floor(dist or 0));
				if M.GrabChest(entry) then
					table.remove(M.Route, 1);
					M.Stats.Chests = M.Stats.Chests + 1;
					return "opening-chest";
				end;
		...
		local boss = wantBosses and M.LiveBoss() or nil;
		local quest = wantQuests and M.BestBeliQuest() or nil;
		local farChest = nil;
		if wantChests then
			farChest = select(1, M.NextRouteStop(here, nil));
		end;

		local pick = M.PickSource(farChest ~= nil, boss ~= nil, quest ~= nil);

		if pick == "Bosses" and boss then
			M.BossLock     = boss;
			M.BossLockAt   = os.clock();
			M.Target       = boss;
			M.ActiveSource = "Bosses";
			M.Plan         = "Boss: " .. boss.Name;
			f.Kill(boss, _G.MoneyFarm);
		...
		if pick == "Quests" and quest then
			M.ActiveSource = "Quests";
			M.Plan = string.format("%s - %s x%d for %s beli", quest.Id, quest.Mob, quest.Kills, S.Short(quest.Beli));
			local enemy = BFFindLiveEnemyLike(quest.Mob);
			if enemy then
				M.Target = enemy;
			...
			if not CheckHasQuest(quest.Mob) and os.clock() >= M.NextQuestAt then
				local giver = S.CachedGiver(quest.Id) or S.FindGiver(quest.Id, centre);
				if giver then
					if M.TweenTo(giver, 6) then
						BFComm("StartQuest", quest.Id, quest.Tier);
						M.NextQuestAt = os.clock() + 2;
						task.wait(.25);
						S.NoteGiverResult(quest.Id, CheckHasQuest(quest.Mob));
						return "starting-beli-quest";
					end;
					return "tweening-to-quest-giver";
				end;
		...
		if farChest then
			M.Target       = farChest.Obj;
			M.ActiveSource = "Chests";
			M.Plan         = "Travelling to a far chest";
			if M.GrabChest(farChest) then
				table.remove(M.Route, 1);
				M.Stats.Chests = M.Stats.Chests + 1;
				return "opening-chest";
			end;
		...
		if wantChests then
			local remaining = #M.LiveChests();
			if remaining <= M.MinChests then
				if M.WaitRespawn and os.clock() < (M.RespawnWaitUntil or 0) then
					M.ActiveSource = nil;
					M.Plan = "Waiting for chests to respawn";
					return "waiting-for-chests";
				end;
				if M.HopWhenEmpty and os.clock() >= M.NextHopAt then
					if M.BossLock == nil then
						M.NextHopAt = os.clock() + 25;
						M.Plan = "Map is out of chests, hopping server";
						if type(Hop) == "function" then pcall(Hop); end;
						return "hopping-server";
					end;
				end;
				if M.WaitRespawn and not M.HopWhenEmpty then
					M.RespawnWaitUntil = os.clock() + 45;
					M.Plan = "Waiting for chests to respawn";
					return "waiting-for-chests";
				end;
			end;
	...
	local _origResetStats = M.ResetStats;
	function M.ResetStats()
		_origResetStats();
		M.ResetEarnings();
		M.Route            = {};
		M.RouteAt          = 0;
		M.BossLock         = nil;
		M.RespawnWaitUntil = 0;
	end;

	local _origMoneyLabels = M.RefreshLabels;
	function M.RefreshLabels()
		_origMoneyLabels();
		if M.StatsLabel and _G.MoneyFarm then
			local parts = {};
			for _, name in ipairs({ "Chests", "Bosses", "Quests" }) do
				local rate = M.SourceRate(name);
				if rate then
					table.insert(parts, string.format("%s %s/h", name, S.Short(rate * 3600)));
				end;
			end;
			if #parts > 0 then
				local extra = string.format("%s beli/hour  |  %s earned  |  %d chests  |  %d bosses\n%s",
					S.Short(M.BeliPerHour()), S.Short(M.Stats.Gained), M.Stats.Chests, M.Stats.Bosses,
					table.concat(parts, "   "));
				if extra ~= M.LastStatsText then
					M.LastStatsText = extra;
					M.StatsLabel:SetText(extra);
				end;
			end;
		end;
	end;
